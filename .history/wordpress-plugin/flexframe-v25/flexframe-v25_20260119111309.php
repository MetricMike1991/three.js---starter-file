...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        $ui_css .= '
            ...
            /* Move button to the right when menu is visible */
            #flexframe-viewer-container .thumbnail-grid-container.menu-visible ~ .menu-hint-tab,
            #flexframe-viewer-container .thumbnail-grid-container.menu-visible .menu-hint-tab,
            .thumbnail-grid-container.menu-visible .menu-hint-tab {
        ...
        $toggle_js = "
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('click', function(e) {
                    if (leftMenuContainer && !leftMenuContainer.contains(e.target) && !leftHintTab.contains(e.target)) {
                        // Don't close if clicking on a dropdown that's shown
                        var openDropdown = document.querySelector('.thumbnail-dropdown.show');
                        if (!openDropdown || !openDropdown.contains(e.target)) {
                            // leftMenuContainer.classList.remove('menu-visible');
                        }
                    }
                    if (rightMenuContainer && !rightMenuContainer.contains(e.target) && !rightHintTab.contains(e.target)) {
                        var openDropdownRight = document.querySelector('.thumbnail-dropdown-right.show');
                        if (!openDropdownRight || !openDropdownRight.contains(e.target)) {
                            // rightMenuContainer.classList.remove('menu-visible');
                        }
                    }
                });
            });
