                        ...
                        <div class="flexframe-advanced-settings" id="logo-advanced-settings" style="display:none;">
                            ...
                            <div class="flexframe-setting-row">
                                <label class="flexframe-checkbox-label">
                                    ...
                                    />
                                    <?php _e('Show Logo on Viewer Background', 'flexframe-viewer'); ?>
                                </label>
                                <?php if (empty($logo_url)) : ?>
                                    <p class="description" style="margin-left: 24px; color: #d63638;">
                                        <?php _e('Upload a logo above to enable this feature.', 'flexframe-viewer'); ?>
                                    </p>
                                <?php else : ?>
                                    <p class="description" style="margin-left: 24px;">
                                        <?php _e('Display your logo as a subtle watermark in the 3D viewer background.', 'flexframe-viewer'); ?>
                                    </p>
                                <?php endif; ?>
                            </div>
                            
                            <div class="flexframe-bg-logo-options" id="bg_logo_options" style="<?php echo $bg_logo_enabled ? '' : 'display:none;'; ?>">
                                <div class="flexframe-setting-row">
                                    <label for="flexframe_bg_logo_size"><?php _e('Background Logo Size', 'flexframe-viewer'); ?></label>
                                    <div class="flexframe-slider-control">
                                        <input 
                                            type="range" 
                                            id="flexframe_bg_logo_size" 
                                            name="flexframe_bg_logo_size" 
                                            value="<?php echo esc_attr($bg_logo_size); ?>" 
                                            min="50" 
                                            max="400" 
                                            step="10"
                                            class="flexframe-slider"
                                        />
                                        <span class="flexframe-slider-value" id="bg_logo_size_value"><?php echo esc_attr($bg_logo_size); ?>px</span>
                                    </div>
                                </div>
                                
                                <div class="flexframe-setting-row">
                                    <label for="flexframe_bg_logo_opacity"><?php _e('Background Logo Opacity', 'flexframe-viewer'); ?></label>
                                    <div class="flexframe-slider-control">
                                        <input 
                                            type="range" 
                                            id="flexframe_bg_logo_opacity" 
                                            name="flexframe_bg_logo_opacity" 
                                            value="<?php echo esc_attr($bg_logo_opacity); ?>" 
                                            min="0.05" 
                                            max="0.5" 
                                            step="0.05"
                                            class="flexframe-slider"
                                        />
                                        <span class="flexframe-slider-value" id="bg_logo_opacity_value"><?php echo esc_attr($bg_logo_opacity * 100); ?>%</span>
                                    </div>
                                    <p class="description">
                                        <?php _e('Lower values create a more subtle watermark effect.', 'flexframe-viewer'); ?>
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="preset-manager">
                                <div class="preset-manager-row">
                                    <div class="preset-load-section">
                                        <label for="flexframe_load_preset"><?php _e('Load Preset:', 'flexframe-viewer'); ?></label>
                                        <select id="flexframe_load_preset" class="preset-select">
                                            ...
                                            <optgroup label="<?php _e('Built-in Themes', 'flexframe-viewer'); ?>">
                                                <option value="builtin:default"><?php _e('Default Theme', 'flexframe-viewer'); ?></option>
                                                <option value="builtin:light"><?php _e('Light Theme', 'flexframe-viewer'); ?></option>
                                                <option value="builtin:dark"><?php _e('Dark Theme', 'flexframe-viewer'); ?></option>
                                                <option value="builtin:branded"><?php _e('Brand Theme', 'flexframe-viewer'); ?></option>
                                            </optgroup>
                                            <?php if (!empty($custom_presets)) : ?>
                                            <optgroup label="<?php _e('Your Saved Presets', 'flexframe-viewer'); ?>">
                                        ...
                                        <button type="button" id="flexframe-delete-preset-btn" class="button button-link-delete" disabled style="display: none;">
    ...
    jQuery(document).ready(function($) {
        ...
        $('input[name="flexframe_material_mode"]').on('change', function() {
            ...
            if (mode === 'preset') {
                ...
                // Keep all custom panel sections EXPANDED when preset is selected
                $('.custom-panel-header').each(function() {
                    var $header = $(this);
                    var targetId = $header.data('target');
                    var $content = $('#' + targetId);
                ...
                // COLLAPSE all custom panel sections when custom is selected
                $('.custom-panel-header').each(function() {
                    var $header = $(this);
                    var targetId = $header.data('target');
                    var $content = $('#' + targetId);
        ...
        // UI Header Settings
        $('#flexframe_player_button_bg_color, #flexframe_player_button_bg_opacity, #flexframe_player_icon_color, #flexframe_menu_bg_color, #flexframe_menu_bg_opacity, #flexframe_menu_text_color').on('input change', updateUIHeaderPreview);
