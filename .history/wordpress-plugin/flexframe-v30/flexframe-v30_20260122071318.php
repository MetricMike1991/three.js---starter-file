...
define('FLEXFRAME_VERSION', '1.29.0');
...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        // Background Logo Watermark settings
        $bg_logo_enabled = get_option('flexframe_bg_logo_enabled', false);
        $bg_logo_size_px = absint(get_option('flexframe_bg_logo_size', 150));
        // Convert pixel size to Three.js scene units (roughly: 100px = 1.5 units)
        $bg_logo_size_units = $bg_logo_size_px / 75;
        $bg_logo_settings = array(
            'enabled' => (bool) $bg_logo_enabled,
            'url' => !empty($logo_url) ? $logo_url : '',
            'size' => $bg_logo_size_units,
            'opacity' => floatval(get_option('flexframe_bg_logo_opacity', 0.15))
        );
        ...
        $settings_data = array(
            ...
            'bgLogoSettings' => $bg_logo_settings,
...
function flexframe_viewer_shortcode($atts) {
    ...
    $bg_logo_opacity = floatval(get_option('flexframe_bg_logo_opacity', 0.15));
    ...
    <div id="flexframe-viewer-container" style="width: <?php echo esc_attr($atts['width']); ?>; height: <?php echo esc_attr($atts['height']); ?>; position: relative; overflow: hidden;">
        ...
        <!-- Background Logo Watermark - positioned behind canvas -->
        <div class="flexframe-bg-logo" style="
            position: absolute;
            left: 50%;
            top: 25%;
            ...
            z-index: 0;
        ">
            <img src="<?php echo esc_url($logo_url); ?>" alt="" style="width: 100%; height: auto;" />
        </div>
        <?php endif; ?>
