...
define('FLEXFRAME_VERSION', '1.30.5');
define('FLEXFRAME_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('FLEXFRAME_PLUGIN_URL', plugin_dir_url(__FILE__));

// Log plugin initialization
flexframe_log('Plugin loaded', array('version' => '1.29.0', 'plugin_url' => plugin_dir_url(__FILE__)));
...
/**
 * Inject critical CSS early in the head to prevent FOUC (Flash of Unstyled Content)
 * This runs at priority 1 to ensure it's one of the first things in <head>
 */
function flexframe_critical_css() {
    global $post;
    
    // Only add critical CSS on pages with the FlexFrame shortcode
    if (!is_a($post, 'WP_Post') || !has_shortcode($post->post_content, 'flexframe_viewer')) {
        return;
    }
    
    // Output critical CSS directly - this prevents the page background from flashing
    echo '<style id="flexframe-critical-css">
        /* FlexFrame Critical CSS - Prevents background flash */
        html, body {
            background: #000000 !important;
            margin: 0 !important;
            padding: 0 !important;
        ...
        /* Hide all potential WordPress theme elements immediately */
        body > *:not(#flexframe-viewer-container):not(script):not(style):not(link) {
...
add_action('wp_head', 'flexframe_critical_css', 1);
...
function flexframe_viewer_shortcode($atts) {
    ...
    <div id="flexframe-viewer-container" style="width: <?php echo esc_attr($atts['width']); ?>; height: <?php echo esc_attr($atts['height']); ?>; position: relative; overflow: hidden;">
        ...
        <div class="thumbnail-grid-container" style="display: none;">
        ...
        <!-- Right Side Menu System -->
        <div class="thumbnail-grid-container-right" style="display: none;">
