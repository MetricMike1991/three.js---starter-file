...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        wp_enqueue_style(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-Cx3ulZED.css',
            array(),
            FLEXFRAME_VERSION
        );
        ...
        wp_register_script(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-DZzz2GWd.js',
            array(),
            FLEXFRAME_VERSION,
            true
        );
...
/**
 * AJAX handler: Save a user-captured screenshot as the custom exercise thumbnail.
 * Accepts a base64-encoded JPEG from the frontend Screen Capture API,
 * uploads it to the Media Library, and updates the custom exercise's thumbnailUrl.
 */
function flexframe_capture_yt_thumbnail() {
    ...
    $file_array = array(
        'name'     => sanitize_file_name('yt-capture-' . $exercise_id . '-' . time() . '.jpg'),
        'tmp_name' => $saved['path'],
    );

    $attachment_id = media_handle_sideload($file_array, 0, 'FlexFrame: Captured thumbnail for ' . $exercise_id);
