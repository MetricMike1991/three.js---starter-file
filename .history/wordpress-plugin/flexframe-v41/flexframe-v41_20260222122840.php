...
define('FLEXFRAME_VERSION', '1.41.472');
...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        wp_enqueue_style(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-DP9zXYFO.css',
            array(),
            FLEXFRAME_VERSION
        );
        ...
        wp_register_script(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-7uVbL0ot.js',
            array(),
            FLEXFRAME_VERSION,
            true
        );
        ...
        $settings_data = array(
            ...
            'ytCaptureNonce' => wp_create_nonce('flexframe_yt_capture')
        );
...
/**
 * AJAX handler: Capture YouTube thumbnail for custom exercise
 * Downloads the YouTube video thumbnail, crops to square, uploads to Media Library,
 * and updates the custom exercise's thumbnailUrl.
 */
function flexframe_capture_yt_thumbnail() {
    check_ajax_referer('flexframe_yt_capture', 'nonce');
    ...
    $exercise_id = sanitize_text_field($_POST['exercise_id']);

    if (!$video_id || !$exercise_id) {
    ...
    if (!$tmp_file) {
        wp_send_json_error('Could not download YouTube thumbnail');
    }

    // Crop to square (center crop using the shorter dimension)
    $editor = wp_get_image_editor($tmp_file);
    if (is_wp_error($editor)) {
        @unlink($tmp_file);
        wp_send_json_error('Image editor not available');
    }
    ...
    $file_array = array(
        'name'     => sanitize_file_name('yt-thumb-' . $video_id . '-' . time() . '.jpg'),
        'tmp_name' => $saved['path'],
    );

    $attachment_id = media_handle_sideload($file_array, 0, 'FlexFrame: YouTube thumbnail for ' . $exercise_id);

    if (is_wp_error($attachment_id)) {
        @unlink($saved['path']);
        wp_send_json_error('Failed to upload to media library');
    }

    $new_url = wp_get_attachment_url($attachment_id);

    // Update the custom exercise option
    $exercises_json = get_option('flexframe_custom_exercises', '[]');
    $exercises      = json_decode($exercises_json, true);
    if (!is_array($exercises)) $exercises = array();

    $updated = false;
    foreach ($exercises as &$ex) {
        if (isset($ex['id']) && $ex['id'] === $exercise_id) {
            $ex['thumbnailUrl'] = $new_url;
            $updated = true;
            break;
        }
    }
    ...
    if ($updated) {
        update_option('flexframe_custom_exercises', wp_json_encode($exercises));
    }
...
add_action('wp_ajax_flexframe_capture_yt_thumbnail', 'flexframe_capture_yt_thumbnail');
