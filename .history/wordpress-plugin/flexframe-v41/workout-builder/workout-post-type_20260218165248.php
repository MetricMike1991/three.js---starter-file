...
function flexframe_save_workout($request) {
    ...
    update_post_meta($post_id, '_flexframe_workout_last_accessed', current_time('mysql'));
...
function flexframe_update_workout($request) {
    ...
    // Touch last accessed timestamp on update
    update_post_meta($post_id, '_flexframe_workout_last_accessed', current_time('mysql'));
...
function flexframe_get_shared_workout($request) {
    ...
    // Touch last accessed timestamp and increment view count on every view
    update_post_meta($post->ID, '_flexframe_workout_last_accessed', current_time('mysql'));
    $views = intval(get_post_meta($post->ID, '_flexframe_workout_view_count', true));
    update_post_meta($post->ID, '_flexframe_workout_view_count', $views + 1);
...
/**
 * AJAX: Get saved workouts (logged-in user workouts) for admin panel
 */
function flexframe_ajax_get_saved_workouts() {
    if (!current_user_can('manage_options')) {
        wp_send_json_error('Permission denied');
    }
    ...
    $args = array(
        ...
        'orderby'        => 'date',
        'order'          => 'DESC',
        'author__not_in' => array(0), // Only logged-in user workouts
    );
    ...
    foreach ($query->posts as $post) {
        $hash = get_post_meta($post->ID, '_flexframe_workout_hash', true);
        $visibility = get_post_meta($post->ID, '_flexframe_workout_visibility', true);
        $last_accessed = get_post_meta($post->ID, '_flexframe_workout_last_accessed', true);
        $duration = get_post_meta($post->ID, '_flexframe_workout_estimated_duration', true);
        $exercises = json_decode(get_post_meta($post->ID, '_flexframe_workout_data', true), true);
        $author = get_userdata($post->post_author);
        $view_count = intval(get_post_meta($post->ID, '_flexframe_workout_view_count', true));
...
add_action('wp_ajax_flexframe_get_saved_workouts', 'flexframe_ajax_get_saved_workouts');

/**
 * AJAX: Delete a saved workout
 */
function flexframe_ajax_delete_saved_workout() {
    if (!current_user_can('manage_options')) {
        wp_send_json_error('Permission denied');
    }

    $id = intval($_POST['workout_id'] ?? 0);
    if (!$id) {
        wp_send_json_error('Invalid ID');
    }
    ...
    $post = get_post($id);
    if (!$post || $post->post_type !== 'flexframe_workout') {
        wp_send_json_error('Workout not found');
    }

    wp_delete_post($id, true);
    wp_send_json_success();
}
add_action('wp_ajax_flexframe_delete_saved_workout', 'flexframe_ajax_delete_saved_workout');
