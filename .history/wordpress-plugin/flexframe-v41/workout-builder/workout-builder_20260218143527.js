...
(function() {
    ...
    function bindEvents() {
        ...
        // Banner toggle (collapse/expand)
        shareBanner?.querySelector('.ffwb-share-banner-toggle')?.addEventListener('click', (e) => {
            e.stopPropagation();
            shareBanner.classList.toggle('ffwb-banner-collapsed');
        });
        // Clicking collapsed banner expands it
        shareBanner?.addEventListener('click', () => {
            if (shareBanner.classList.contains('ffwb-banner-collapsed')) {
                shareBanner.classList.remove('ffwb-banner-collapsed');
            }
        });
    ...
    async function loadSharedWorkout(hash) {
        try {
            ...
            // Auto-collapse banner after 5 seconds
            setTimeout(() => {
                if (shareBanner && !shareBanner.classList.contains('ffwb-banner-collapsed')) {
                    shareBanner.classList.add('ffwb-banner-collapsed');
                }
            }, 5000);
    ...
    function editSharedWorkout() {
        ...
        renderExerciseList();
        showToast('Edit mode — make your changes!');
    }
