...
function flexframe_coach_system_prompt() {
    ...
    return "You are FlexFrame Coach, a friendly, knowledgeable personal trainer chat assistant on the {$gym} workout builder page.\n\n" .
        ...
        "- Pick exercises ONLY from the catalogue list provided in the next system message. Use each exercise's exact `id` value as `exerciseId`.\n" .
        "- 4-10 exercises total for the session, ordered logically (compound lifts first, accessories after).\n" .
        "- Sets: integer 1-6. Reps: a string like \"8\", \"8-10\", \"30s\", \"AMRAP\". Rest: integer seconds (30-300).\n" .
        "- RIR (reps in reserve): string \"0\"-\"4\", default \"2\".\n" .
        "- Notes: short coaching cue (max ~120 chars), or empty string.\n" .
        "- Supersets: give consecutive exercises the SAME `groupId` string (e.g. \"g1\", \"g2\"). Stand-alone exercises use `null`.\n" .
                    ...
                    'exercises' => array(
                        ...
                        'items'    => array(
                            ...
                            'properties'           => array(
                                ...
                                'reps'       => array('type' => 'string', 'description' => 'Reps or duration, e.g. "8-10", "30s", "AMRAP".'),
...
function flexframe_validate_coach_workout($args, $catalogue) {
    ...
    foreach ($exercises as $ex) {
        ...
        if (strlen($notes) > 200) $notes = substr($notes, 0, 200);
