...
define('FLEXFRAME_VERSION', '1.45.6');
...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        wp_register_script(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-Djgw8K6J.js',
            array(),
            FLEXFRAME_VERSION,
            true
        );
        ...
        $settings_data = array(
            ...
            'viewerPageUrl' => esc_url(get_option('flexframe_viewer_page_url', '')),
    ...
    <div id="flexframe-viewer-container" style="width: <?php echo esc_attr($atts['width']); ?>; height: <?php echo esc_attr($atts['height']); ?>; position: relative; overflow: hidden; border-radius: 0 !important;">
        ...
        (function(){
            ...
            setInterval(function() {
                // Don't override visibility if screenshot panel is open
                if (document.body.classList.contains('ff-panel-open')) return;
                ws.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;margin:0;padding:0;pointer-events:none;z-index:1;overflow:hidden;box-sizing:border-box;';
                s.position = 'absolute';
                s.top = posYcss + '%';
                s.left = posX + '%';
...
function flexframe_embed_mode_redirect() {
    ...
    $js_url = FLEXFRAME_PLUGIN_URL . 'assets/assets/index-Djgw8K6J.js';
...
function flexframe_create_annotations_table() {
    global $wpdb;
    $table = $wpdb->prefix . 'flexframe_annotations';
    $charset_collate = $wpdb->get_charset_collate();
    $sql = "CREATE TABLE IF NOT EXISTS $table (
        id bigint(20) NOT NULL AUTO_INCREMENT,
        exercise_id varchar(255) NOT NULL DEFAULT '',
        label varchar(255) NOT NULL DEFAULT '',
        body_text text NOT NULL,
        bone_name varchar(255) NOT NULL DEFAULT '',
        ...
        PRIMARY KEY (id),
        KEY exercise_id (exercise_id(191))
    ) $charset_collate;";
    require_once ABSPATH . 'wp-admin/includes/upgrade.php';
    dbDelta($sql);
}
add_action('plugins_loaded', 'flexframe_create_annotations_table');

function flexframe_register_annotations_api() {
    ...
    // POST /flexframe/v1/annotations — logged-in only
    register_rest_route('flexframe/v1', '/annotations', array(
        'methods'             => 'POST',
        'callback'            => function(WP_REST_Request $req) use ($table_check) {
            global $wpdb;
            ...
            $data = array(
                ...
                'body_text'   => sanitize_textarea_field($req->get_param('body_text')),
                'bone_name'   => sanitize_text_field($req->get_param('bone_name')),
                'offset_x'    => floatval($req->get_param('offset_x')),
                'offset_y'    => floatval($req->get_param('offset_y')),
                'offset_z'    => floatval($req->get_param('offset_z')),
                'created_by'  => get_current_user_id(),
            );
            if (!$data['exercise_id'] || !$data['label']) {
                return new WP_Error('missing_fields', 'exercise_id and label required', array('status' => 400));
            }
            $wpdb->insert($table, $data);
            $id = $wpdb->insert_id;
            $row = $wpdb->get_row($wpdb->prepare("SELECT id, exercise_id, label, body_text, bone_name, offset_x, offset_y, offset_z FROM $table WHERE id = %d", $id), ARRAY_A);
            return rest_ensure_response($row);
        },
        'permission_callback' => function() { return is_user_logged_in(); },
    ));

    // PUT /flexframe/v1/annotations/{id} — logged-in + owner or admin
    register_rest_route('flexframe/v1', '/annotations/(?P<id>\d+)', array(
        'methods'             => 'PUT',
        'callback'            => function(WP_REST_Request $req) use ($table_check) {
            global $wpdb;
            $table = $table_check();
            ...
            if (!$row) return new WP_Error('not_found', 'Annotation not found', array('status' => 404));
            if (!current_user_can('manage_options') && intval($row['created_by']) !== get_current_user_id()) {
            ...
            $updated = $wpdb->get_row($wpdb->prepare("SELECT id, exercise_id, label, body_text, bone_name, offset_x, offset_y, offset_z FROM $table WHERE id = %d", $id), ARRAY_A);
            return rest_ensure_response($updated);
        },
        'permission_callback' => function() { return is_user_logged_in(); },
    ));

    // DELETE /flexframe/v1/annotations/{id} — logged-in + owner or admin
    register_rest_route('flexframe/v1', '/annotations/(?P<id>\d+)', array(
        'methods'             => 'DELETE',
        'callback'            => function(WP_REST_Request $req) use ($table_check) {
            global $wpdb;
...
function flexframe_register_ai_render_api() {
    register_rest_route('flexframe/v1', '/ai-render', array(
        ...
        'permission_callback' => function (WP_REST_Request $request) {
            ...
            // Allow anonymous ONLY for workout-mode image generation, and only when
            // the admin has explicitly enabled the A.I Image button for logged-out users.
            $mode = (string) $request->get_param('mode');
            if ($mode === 'workout' && (bool) get_option('flexframe_ai_image_logged_out_enabled', false)) {
                return true;
            }
            return false;
        },
 ...
 */
function flexframe_ai_default_workout_caption_template() {
    return "Write a SHORT Instagram caption about the workout '{workoutName}' from {gymName}.\n\n" .
        "Here are the exercises in the workout (do NOT list them all in the caption — they're context only):\n{exerciseList}\n\n" .
        ...
        "- Around 3-5 emojis total, placed naturally in the hook and the description, NOT in the call to action.\n" .
        "- Separate the 4 blocks above with a single blank line so it reads well on Instagram.\n" .
        "- Do NOT include preambles, headings, or labels like 'Hook:' or 'Fact:'. Output the caption only.";
}
...
function flexframe_handle_ai_caption(WP_REST_Request $request) {
    ...
    if ($mode !== 'workout') { $mode = 'exercise'; }

    $gym_name = get_bloginfo('name');
    if (empty($gym_name)) { $gym_name = 'this gym'; }
    $site_url = home_url('/');
    ...
    if ($mode === 'workout') {
        ...
        if ($provider === 'gemini') {
            return flexframe_ai_caption_gemini($prompt, $workout_name, $gym_name);
        }
        return flexframe_ai_caption_openai($prompt, $workout_name, $gym_name);
    }
 ...
 */
function flexframe_coach_system_prompt($strict = true) {
    ...
    $intro = "You are {$bot_name}, a friendly, knowledgeable personal trainer chat assistant on the {$gym} workout builder page.\n\n";
    ...
    $scope = $strict
        ...
        : ("SCOPE:\n" .
            "- Your primary job is to help the user build a single workout session via the `propose_workout` tool.\n" .
            "- You may also discuss general fitness, nutrition, recovery, and related questions in a helpful, conversational way. Be sensible: never give specific medical advice or recommend specific drugs/doses; suggest the user consult a qualified professional for medical or prescription matters.\n\n");
