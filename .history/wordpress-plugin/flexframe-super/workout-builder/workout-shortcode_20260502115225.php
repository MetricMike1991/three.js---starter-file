...
function flexframe_enqueue_workout_builder_assets() {
    ...
    if ($coach_enabled) {
        // html2canvas for the A.I Image button (only when image feature is enabled)
        if ($ai_image_eff) {
            wp_enqueue_script(
                'html2canvas',
...
function flexframe_workout_builder_shortcode($atts) {
    ...
    $coach_image_li   = (bool) get_option('flexframe_ai_image_logged_in_enabled', true);
    $coach_image_lo   = (bool) get_option('flexframe_ai_image_logged_out_enabled', false);
    $coach_image_eff  = is_user_logged_in() ? $coach_image_li : $coach_image_lo;
    ...
    <div id="flexframe-ai-coach" class="ffc-root<?php echo $coach_bot_avatar ? ' ffc-has-avatar' : ''; ?>">
        ...
        <div class="ffc-panel" role="dialog" aria-label="<?php echo esc_attr(sprintf(__('%s chat', 'flexframe-viewer'), $coach_bot_name)); ?>">
            ...
            <div class="ffc-actions" role="toolbar" aria-label="Quick actions">
                ...
                <?php if ($coach_image_eff): ?>
                <button type="button" class="ffc-action ffc-action-image" data-action="ai-image" title="Take a screenshot of your current workout and have AI turn it into a branded Instagram-ready infographic.">A.I Image</button>
                <?php endif; ?>
                <?php if ($coach_wod_on): ?>
