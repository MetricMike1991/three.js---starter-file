...
define('FLEXFRAME_VERSION', '1.38.0');
define('FLEXFRAME_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('FLEXFRAME_PLUGIN_URL', plugin_dir_url(__FILE__));

// Log plugin initialization
flexframe_log('Plugin loaded', array('version' => '1.38.0', 'plugin_url' => plugin_dir_url(__FILE__)));
...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        wp_enqueue_style(
            'flexframe-vite-style',
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-MgKN09E4.css',
            array(),
            FLEXFRAME_VERSION
        );
        
        // Add UI customization CSS inline
        wp_add_inline_style('flexframe-vite-style', $ui_css);
        
        // Register Vite-generated JavaScript bundle (must register before localizing)
        wp_register_script(
            'flexframe-viewer-script',
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-ctVI-QaY.js',
            array(),
            FLEXFRAME_VERSION,
            true
        );
        
        // Add filter to make script load as ES module
        add_filter('script_loader_tag', 'flexframe_add_module_type', 10, 3);
...
/**
 * Add type="module" to the FlexFrame viewer script
 */
function flexframe_add_module_type($tag, $handle, $src) {
