...
/**
 * Safely convert a hex color to an RGB array [r, g, b].
 * Returns a fallback (default [0,0,0]) if the value isn't a valid hex color.
 * This prevents fatal errors from sscanf returning null/incomplete arrays.
 */
function flexframe_safe_hex_to_rgb($hex, $fallback = array(0, 0, 0)) {
    $hex = flexframe_sanitize_color($hex);
    $result = sscanf($hex, "#%02x%02x%02x");
    if (!is_array($result) || count($result) < 3 || $result[0] === null || $result[1] === null || $result[2] === null) {
        return $fallback;
    }
    return array(intval($result[0]), intval($result[1]), intval($result[2]));
}

/**
 * Sanitize a color value to ensure it's always a valid #rrggbb hex string.
 * Handles empty strings, rgba(), 3-digit hex, 8-digit hex+alpha, values
 * without # prefix, and esc_attr-encoded values. Returns $fallback if not salvageable.
 */
function flexframe_sanitize_color($color, $fallback = '#000000') {
    if (empty($color) || !is_string($color)) {
        return $fallback;
    }
    $color = trim($color);

    // If it's an rgba/rgb string, try to extract hex — or just return fallback
...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        $primary_color = flexframe_sanitize_color(get_option('flexframe_primary_color', '#f50000'), '#f50000');
        $menu_bg_color = flexframe_sanitize_color(get_option('flexframe_menu_bg_color', '#000000'), '#000000');
        $menu_accent_color = flexframe_sanitize_color(get_option('flexframe_menu_accent_color', '#f50000'), '#f50000');
        $menu_bg_rgb = flexframe_safe_hex_to_rgb($menu_bg_color);
        $menu_bg_opacity = floatval(get_option('flexframe_menu_bg_opacity', 0.9));
        
        // Get V2 menu settings
        $menu_v2_bg_color = flexframe_sanitize_color(get_option('flexframe_menu_v2_bg_color', '#1a1a1a'), '#1a1a1a');
        $menu_v2_bg_opacity = floatval(get_option('flexframe_menu_v2_bg_opacity', 0.95));
        $menu_v2_accent_color = flexframe_sanitize_color(get_option('flexframe_menu_v2_accent_color', $primary_color), $primary_color);
        $menu_v2_text_color = flexframe_sanitize_color(get_option('flexframe_menu_v2_text_color', '#ffffff'), '#ffffff');
        ...
        $menu_v2_heading_bg_color = flexframe_sanitize_color(get_option('flexframe_menu_v2_heading_bg_color', $primary_color), $primary_color);
        $menu_v2_heading_bg_opacity = floatval(get_option('flexframe_menu_v2_heading_bg_opacity', $menu_v2_bg_opacity));
        $menu_v2_heading_bg_rgb = flexframe_safe_hex_to_rgb($menu_v2_heading_bg_color);
        ...
        $menu_v2_bg_rgb = flexframe_safe_hex_to_rgb($menu_v2_bg_color, array(26, 26, 26));
        $menu_v2_bg_rgba = 'rgba(' . implode(', ', $menu_v2_bg_rgb) . ', ' . $menu_v2_bg_opacity . ')';
        $menu_v2_text_rgb = flexframe_safe_hex_to_rgb($menu_v2_text_color, array(255, 255, 255));
        ...
        $menu_v2_search_input_bg_color = flexframe_sanitize_color(get_option('flexframe_menu_v2_search_input_bg_color', $menu_v2_bg_color), $menu_v2_bg_color);
        $menu_v2_search_input_bg_rgb = flexframe_safe_hex_to_rgb($menu_v2_search_input_bg_color, array(26, 26, 26));
        ...
        $menu_v2_accent_rgb = flexframe_safe_hex_to_rgb($menu_v2_accent_color, array(245, 0, 0));
        ...
        $spinner_color = flexframe_sanitize_color(get_option('flexframe_spinner_color', '#00f510'), '#00f510');
        ...
        $player_bg_color = flexframe_sanitize_color(get_option('flexframe_player_bg_color', '#828282'), '#828282');
        $player_bg_opacity = floatval(get_option('flexframe_player_bg_opacity', 0));
        $player_button_bg_color = flexframe_sanitize_color(get_option('flexframe_player_button_bg_color', '#f50000'), '#f50000');
        $player_button_bg_opacity = floatval(get_option('flexframe_player_button_bg_opacity', 0.8));
        $player_icon_color = flexframe_sanitize_color(get_option('flexframe_player_icon_color', '#ffffff'), '#ffffff');
        $player_accent_color = flexframe_sanitize_color(get_option('flexframe_player_accent_color', '#f50000'), '#f50000');
        $player_always_visible = get_option('flexframe_player_always_visible', 'no') === 'yes';
        $menu_bg_color = flexframe_sanitize_color(get_option('flexframe_menu_bg_color', '#000000'), '#000000');
        $menu_bg_opacity = floatval(get_option('flexframe_menu_bg_opacity', 0.9));
        $menu_text_color = flexframe_sanitize_color(get_option('flexframe_menu_text_color', '#ffffff'), '#ffffff');
        ...
        $thumbnail_label_color = flexframe_sanitize_color(get_option('flexframe_thumbnail_label_color', '#000000'), '#000000');
        ...
        $player_bg_rgb = flexframe_safe_hex_to_rgb($player_bg_color, array(130, 130, 130));
        $player_button_bg_rgb = flexframe_safe_hex_to_rgb($player_button_bg_color, array(245, 0, 0));
        $menu_bg_rgb = flexframe_safe_hex_to_rgb($menu_bg_color);
        $menu_text_rgb = flexframe_safe_hex_to_rgb($menu_text_color, array(255, 255, 255));
        $primary_rgb = flexframe_safe_hex_to_rgb($primary_color, array(245, 0, 0));
        $thumbnail_label_rgb = flexframe_safe_hex_to_rgb($thumbnail_label_color);
        ...
        wp_register_script(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-CRuIIXIH.js',
            array(),
            FLEXFRAME_VERSION,
            true
        );
