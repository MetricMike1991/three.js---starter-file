 ...
 * When a demo page is loaded, override global FlexFrame settings
 * with the demo page's assigned theme preset settings.
 */
function flexframe_maybe_apply_demo_overrides() {
    ...
    // Build the settings map from the preset
    $override_settings = array();
    
    if (strpos($preset_key, 'custom:') === 0) {
        // Custom preset - load from saved presets
        $custom_presets = get_option('flexframe_custom_presets', array());
        $preset_id = str_replace('custom:', '', $preset_key);
        if (isset($custom_presets[$preset_id]) && isset($custom_presets[$preset_id]['settings'])) {
            $s = $custom_presets[$preset_id]['settings'];
            $override_settings = flexframe_map_preset_to_options($s);
        }
    } else {
        // Built-in preset - define the settings inline
        $built_in = flexframe_get_builtin_preset($preset_key);
        if ($built_in) {
    ...
    if (empty($override_settings)) return;
    
    // Apply pre_option filters for each setting
    foreach ($override_settings as $option_name => $value) {
        $filter_value = $value; // Capture for closure
        add_filter("pre_option_{$option_name}", function() use ($filter_value) {
            return $filter_value;
        });
    }
    
    flexframe_log('Demo page overrides applied', array('post_id' => $post->ID, 'preset' => $preset_key));
}
...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        wp_register_script(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-BmvWdrKN.js',
            array(),
            FLEXFRAME_VERSION,
            true
        );
