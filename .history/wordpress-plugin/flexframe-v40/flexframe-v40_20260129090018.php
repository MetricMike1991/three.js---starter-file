...
function flexframe_enqueue_assets() {
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'flexframe_viewer')) {
        ...
        wp_register_script(
            ...
            FLEXFRAME_PLUGIN_URL . 'assets/assets/index-BT1LfUEh.js',
            array(),
            FLEXFRAME_VERSION,
            true
        );
...
function flexframe_viewer_shortcode($atts) {
    ...
    <div id="flexframe-viewer-container" style="width: <?php echo esc_attr($atts['width']); ?>; height: <?php echo esc_attr($atts['height']); ?>; position: relative; overflow: hidden; border-radius: 0 !important;">
        ...
        <button id="fullscreen-btn" class="fullscreen-btn" title="Toggle Fullscreen">
            <svg class="fullscreen-enter-icon" viewBox="0 0 24 24" width="18" height="18" fill="white">
                <!-- Expand icon - corner brackets pointing outward -->
                <path d="M5 5h5v2H7v3H5V5zm14 0v5h-2V7h-3V5h5zM5 19v-5h2v3h3v2H5zm14 0h-5v-2h3v-3h2v5z"/>
            </svg>
            <svg class="fullscreen-exit-icon" viewBox="0 0 24 24" width="18" height="18" fill="white" style="display: none;">
                <!-- Contract icon - corner brackets pointing inward -->
                <path d="M10 10H5V8h3V5h2v5zm4 0V5h2v3h3v2h-5zM10 14v5H8v-3H5v-2h5zm9 0v2h-3v3h-2v-5h5z"/>
            </svg>
        </button>
