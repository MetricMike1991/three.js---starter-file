...
// Helper function to get the base URL for assets (WordPress plugin support)
function getAssetUrl(relativePath) {
    // Check if running in WordPress
    if (typeof flexframeSettings !== 'undefined' && flexframeSettings.pluginUrl) {
        return flexframeSettings.pluginUrl + 'assets/' + relativePath;
    }
...
class ThreeJSApp {
        ...
        document.addEventListener('exercisesSelected', async (e) => {
            ...
            if (exercise.configUrl) {
                try {
                    ...
                    const configUrlWithAsset = getAssetUrl(exercise.configUrl);
                    const configUrlWithCache = configUrlWithAsset + cacheBuster;
    ...
    setupGUIVisibilityToggle() {
        ...
        // Add click listener for material selection using renderer's dom element
        this.renderer.domElement.addEventListener('click', (event) => {
            ...
            if (intersects.length > 0) {
                // Find first non-PointsMaterial object
                for (let i = 0; i < intersects.length; i++) {
                    const object = intersects[i].object;
                    if (object.material && object.material.type !== 'PointsMaterial') {
                        this.selectedMaterial = object.material;
                        console.log(`✅ Material selected: ${object.material.name || 'Unnamed'} (${object.material.type})`);
                        break;
                    }
                }
            }
        ...
        window.addEventListener('keydown', (event) => {
            ...
            // Press M to log material settings
            if (event.key === 'm' || event.key === 'M') {
                if (this.selectedMaterial) {
                    console.log('═══════════════════════════════════════════');
                    console.log(`📋 MATERIAL SETTINGS: ${this.selectedMaterial.name || 'Unnamed'}`);
                    console.log('═══════════════════════════════════════════');
                    console.log('Basic Properties:');
                    console.log('  type:', this.selectedMaterial.type);
                    console.log('  name:', this.selectedMaterial.name);
                    console.log('  color:', '#' + this.selectedMaterial.color?.getHexString());
                    console.log('  opacity:', this.selectedMaterial.opacity);
                    console.log('  transparent:', this.selectedMaterial.transparent);
                    console.log('  side:', this.selectedMaterial.side === THREE.FrontSide ? 'FrontSide' : 
                                         this.selectedMaterial.side === THREE.BackSide ? 'BackSide' : 'DoubleSide');
                    
                    console.log('\nPhysical Properties:');
                    console.log('  roughness:', this.selectedMaterial.roughness);
                    console.log('  metalness:', this.selectedMaterial.metalness);
                    console.log('  envMapIntensity:', this.selectedMaterial.envMapIntensity);
                    
                    if (this.selectedMaterial.emissive) {
                        console.log('\nEmissive:');
                        console.log('  emissive:', '#' + this.selectedMaterial.emissive.getHexString());
                        console.log('  emissiveIntensity:', this.selectedMaterial.emissiveIntensity);
                    }
                    
                    if (this.selectedMaterial.transmission !== undefined) {
                        console.log('\nTransmission/Glass:');
                        console.log('  transmission:', this.selectedMaterial.transmission);
                        console.log('  thickness:', this.selectedMaterial.thickness);
                        console.log('  ior:', this.selectedMaterial.ior);
                        console.log('  attenuationDistance:', this.selectedMaterial.attenuationDistance);
                        console.log('  attenuationColor:', this.selectedMaterial.attenuationColor ? 
                            '#' + this.selectedMaterial.attenuationColor.getHexString() : 'N/A');
                    }
                    
                    console.log('\nEnvironment Map:');
                    console.log('  envMap:', this.selectedMaterial.envMap ? '✓ Present' : '✗ None');
                    if (this.selectedMaterial.envMap) {
                        console.log('  envMap type:', this.selectedMaterial.envMap.type);
                        console.log('  envMap mapping:', this.selectedMaterial.envMap.mapping);
                    }
                    
                    if (this.selectedMaterial.clearcoat !== undefined) {
                        console.log('\nClearcoat:');
                        console.log('  clearcoat:', this.selectedMaterial.clearcoat);
                        console.log('  clearcoatRoughness:', this.selectedMaterial.clearcoatRoughness);
                    }
                    
                    if (this.selectedMaterial.sheen !== undefined) {
                        console.log('\nSheen:');
                        console.log('  sheen:', this.selectedMaterial.sheen);
                        console.log('  sheenRoughness:', this.selectedMaterial.sheenRoughness);
                        console.log('  sheenColor:', '#' + this.selectedMaterial.sheenColor?.getHexString());
                    }
                    
                    console.log('\nDepth & Blending:');
                    console.log('  depthWrite:', this.selectedMaterial.depthWrite);
                    console.log('  depthTest:', this.selectedMaterial.depthTest);
                    console.log('  blending:', this.selectedMaterial.blending);
                    console.log('  alphaTest:', this.selectedMaterial.alphaTest);
                    
                    console.log('\nTextures:');
                    console.log('  map:', this.selectedMaterial.map ? '✓' : '✗');
                    console.log('  normalMap:', this.selectedMaterial.normalMap ? '✓' : '✗');
                    console.log('  roughnessMap:', this.selectedMaterial.roughnessMap ? '✓' : '✗');
                    console.log('  metalnessMap:', this.selectedMaterial.metalnessMap ? '✓' : '✗');
                    console.log('  emissiveMap:', this.selectedMaterial.emissiveMap ? '✓' : '✗');
                    console.log('  bumpMap:', this.selectedMaterial.bumpMap ? '✓' : '✗');
                    if (this.selectedMaterial.bumpMap) {
                        console.log('  bumpScale:', this.selectedMaterial.bumpScale);
                    }
                    console.log('═══════════════════════════════════════════');
                } else {
                    console.log('❌ No material selected. Click on an object first, then press M.');
                }
            }
        });
    ...
    applyCustomTextures(model, customTextures) {
        // Check for WordPress logo settings first (global override)
        const wpLogoUrl = typeof flexframeSettings !== 'undefined' ? flexframeSettings.logoUrl : null;
        const wpLogoThreshold = typeof flexframeSettings !== 'undefined' ? flexframeSettings.logoThreshold : 0.95;
        
        // If WordPress logo exists, override the LOGO material in customTextures
        if (wpLogoUrl && wpLogoUrl.trim() !== '') {
            console.log('🔧 WordPress global logo detected, overriding config logo');
            customTextures = { ...customTextures, LOGO: wpLogoUrl };
        }
                    ...
                    mats.forEach((mat) => {
                        if (mat.name === materialName) {
                            ...
                            this.textureLoader.load(cacheBustedUrl, (texture) => {
                                ...
                                // Use WordPress threshold if this is LOGO material, otherwise default
                                if (materialName === 'LOGO' && wpLogoUrl) {
                                    mat.alphaTest = wpLogoThreshold;
                                    console.log(`✅ Using WordPress logo threshold: ${wpLogoThreshold}`);
                                } else {
                                    mat.alphaTest = 0.95;
                                }
