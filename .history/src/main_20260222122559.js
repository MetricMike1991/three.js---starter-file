...
class ThreeJSApp {
    ...
    /**
     * Handle YouTube thumbnail capture — show preview and confirm update
     */
    handleYouTubeCapture() {
        const videoId = this._ytVideoId;
        const exerciseId = this._ytExerciseId;
        const exerciseName = this._ytExerciseName;
        if (!videoId || !exerciseId) return;
        
        // YouTube thumbnail URL (highest quality available)
        ...
        modal.innerHTML = `
            ...
            <div class="ffx-capture-dialog">
                ...
                <p class="ffx-capture-subtitle">Use the YouTube thumbnail for <strong>${exerciseName}</strong>?</p>
                <div class="ffx-capture-preview">
                    <img src="${thumbUrl}" alt="Thumbnail preview" onerror="this.src='${fallbackUrl}'">
                </div>
                <p class="ffx-capture-note">The image will be cropped to a square and saved to your Media Library.</p>
        ...
        modal.querySelector('.ffx-capture-confirm').addEventListener('click', () => {
            ...
            const actions = modal.querySelector('.ffx-capture-actions');
            ...
            fetch(ws.ajaxUrl, { method: 'POST', body: formData })
                ...
                .then(resp => {
                    if (resp.success) {
                        ...
                        `;
                        // Update the thumbnail in the current session's menu if possible
                        ...
                        confirmBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg> Update Thumbnail`;
                        status.style.display = 'block';
