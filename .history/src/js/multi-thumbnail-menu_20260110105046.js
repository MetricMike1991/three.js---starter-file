...
export class MultiThumbnailMenuSystem {
    ...
    initializeMenus() {
        ...
        // Set Information menu to stay open when clicking outside
        this.menus.information.settings.keepOpen = true;
    }
    ...
    setupGlobalListeners() {
        ...
        document.addEventListener('closeAllThumbnailMenus', (e) => {
            ...
            Object.entries(this.menus).forEach(([type, menu]) => {
                // Don't close Information menu (it has keepOpen enabled) or the excepted menu
                if (type !== exceptMenu && type !== 'information') {
                    menu.closeMenu();
                }
            });
        });
            ...
            if (hintTab) {
                hintTab.addEventListener('click', (e) => {
                    ...
                    if (menuContainer.classList.contains('menu-visible') || 
                        ...
                        // Close all menus except Information menu (which has keepOpen enabled)
                        Object.values(this.menus).forEach(menu => {
                            if (menu.isOpen && menu.menuType !== 'information') {
                                menu.closeMenu();
                            }
                        });
                        // Only hide container if Information menu is not open
                        if (!this.menus.information.isOpen) {
        ...
        document.addEventListener('click', (e) => {
            if (menuContainer && !menuContainer.contains(e.target)) {
                ...
                if (anyMenuOpen) {
                    // Close open menus that don't have keepOpen enabled
                    Object.values(this.menus).forEach(menu => {
                        if (menu.isOpen && !menu.settings.keepOpen) {
                            menu.closeMenu();
                        }
                    });
                    
                    // Only remove menu visibility if no keepOpen menus remain open
                    const anyKeepOpenMenuOpen = Object.values(this.menus).some(menu => 
                        menu.isOpen && menu.settings.keepOpen
                    );
                    if (!anyKeepOpenMenuOpen) {
                        menuContainer.classList.remove('menu-visible');
                    }
                }
            }
