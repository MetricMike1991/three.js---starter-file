...
class ThemeEditor {
    constructor(app) {
        console.log('[ThemeEditor] Constructor called with app:', app);
        this.app = app;
        this.isOpen = false;
        this.panel = null;
    ...
    applyPrimaryColorToElements(color) {
        ...
        // 1. Update Animation Player accent color and button background
        if (this.app.animationPlayer) {
            // Update scrubber color (accent color)
            this.currentSettings.playerAccentColor = color;
            this.app.playerStyleParams.scrubberColor = color;
            this.app.updatePlayerScrubberColor(color);
            
            // Update button color (background color for buttons)
            this.currentSettings.playerButtonColor = color;
            this.app.playerStyleParams.buttonColor = color;
            this.app.updatePlayerButtonColor(color);
            
            console.log('[ThemeEditor] Updated animation player colors');
        }
        
        // 2. Update Spinner Color
        if (this.app.updateSpinnerColor) {
            this.currentSettings.spinnerColor = color;
            this.app.updateSpinnerColor(color);
            console.log('[ThemeEditor] Updated spinner color');
        }
    ...
    addStyles() {
        ...
        style.textContent = `
            ...
            .te-save-section-top > div:first-child {
                display: flex;
                gap: 10px;
            }
