...
class ThumbnailDropdownMenu {
    ...
    updateGlobalFilterBox() {
        const selectedMuscle = window.menuManager?.selectedMuscle;
        const selectedEquipment = window.menuManager?.selectedEquipment;
        
        const wrapper = document.getElementById('activeFiltersWrapper');
        const filterBox = document.getElementById('globalFilterBox');
        
        if (!wrapper || !filterBox) return;
        
        // Show or hide wrapper based on filters
        if (selectedMuscle || selectedEquipment) {
            ...
            filterBox.querySelectorAll('.filter-clear').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    if (window.menuManager.menus.exercises) {
                        window.menuManager.menus.exercises.filterDataForMenu();
                    }
                });
            });
        } else {
