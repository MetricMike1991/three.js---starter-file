...
export class AnimationPlayer {
    ...
    initializeScaling() {
        // Simple resize listener to maintain 95% width
        window.addEventListener('resize', () => {
            console.log('=== RESIZE EVENT TRIGGERED ===');
            
            // Ensure player stays at 95% width
            if (this.container) {
                const playerControlsDiv = this.container.querySelector('.player-controls');
                
                console.log('Before resize fixes:');
                console.log('Container width:', this.container.offsetWidth);
                console.log('Container style.width:', this.container.style.width);
                if (playerControlsDiv) {
                    console.log('Player-controls width:', playerControlsDiv.offsetWidth);
                    console.log('Player-controls gap:', window.getComputedStyle(playerControlsDiv).gap);
                }
                
                this.container.style.width = '95%';
                // Remove any transform scaling that might interfere
                this.container.style.transform = this.container.classList.contains('visible') ? 'translateY(0)' : 'translateY(100%)';
                
                console.log('After width fix:');
                console.log('Container width:', this.container.offsetWidth);
                console.log('Container style.width:', this.container.style.width);
                
                // Force layout refresh by triggering reflow
                ...
                if (playerControlsDiv) {
                    ...
                    // Log flex children dimensions
                    const playerLeft = playerControlsDiv.querySelector('.player-left');
                    const playerCenter = playerControlsDiv.querySelector('.player-center');
                    const playerRight = playerControlsDiv.querySelector('.player-right');
                    
                    if (playerLeft) console.log('Player-left width:', playerLeft.offsetWidth);
                    if (playerCenter) console.log('Player-center width:', playerCenter.offsetWidth);
                    if (playerRight) console.log('Player-right width:', playerRight.offsetWidth);
                }
                console.log('=== RESIZE EVENT COMPLETE ===\n');
            }
        });
    }
    
    updatePlayerScale() {
        // Only handle visibility, not scaling - width is fixed at 95%
