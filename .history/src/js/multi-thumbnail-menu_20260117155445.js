...
class ThumbnailDropdownMenu {
    ...
    async loadExerciseData() {
        try {
            const exercisesUrl = getAssetUrl('data/exercises.json');
            console.log('🔍 Loading exercises.json from:', exercisesUrl);
            const response = await fetch(exercisesUrl);
            this.allExercises = await response.json();
            console.log('📋 Loaded exercises data:', this.allExercises);
            console.log('🎯 Deadlift config URL:', this.allExercises.find(e => e.id === 'barbell_deadlift')?.configUrl);
            this.filterDataForMenu();
