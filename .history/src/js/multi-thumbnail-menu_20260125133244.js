...
export class MultiThumbnailMenuSystem {
    ...
    toggleLeftMenu() {
        ...
        const isMobile = window.innerWidth <= 768;
        const isOpen = menuContainer.classList.contains('mobile-open') || menuContainer.classList.contains('menu-visible');
        ...
        if (isOpen) {
            ...
            menuContainer.classList.remove('menu-visible');
            if (isMobile) {
                menuContainer.style.left = '-260px';
            }
            this.updateMobileToggleIcon(false);
            ...
            if (rightMenu && (rightMenu.classList.contains('mobile-open') || rightMenu.classList.contains('menu-visible'))) {
                rightMenu.classList.remove('mobile-open');
                rightMenu.classList.remove('menu-visible');
                if (isMobile) {
                    rightMenu.style.right = '-260px';
                }
                // Close all right menu dropdowns
                document.dispatchEvent(new CustomEvent('closeAllRightMenus', { 
                    detail: { except: null } 
                }));
            }
