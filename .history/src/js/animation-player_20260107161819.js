...
export class AnimationPlayer {
    constructor() {
        ...
        // Initialize scaling constraints early
        this.minScale = 0.6;  // Minimum scale (60%)
        this.maxScale = 1.0;  // Maximum scale (100%)
    ...
    initializeScaling() {
        ...
        // Debounce resize events to prevent rapid-fire scaling
        let resizeTimeout;
        window.addEventListener('resize', () => {
            clearTimeout(resizeTimeout);
            resizeTimeout = setTimeout(() => {
                this.refreshPlayerScaling();
            }, 150);
        });
    }
    
    refreshPlayerScaling() {
        // Completely reset transforms to clean state
        if (this.container) {
            this.container.style.transform = 'none';
            this.container.style.transformOrigin = 'initial';
            
            // Force reflow to clear any cached values
            this.container.offsetHeight;
            
            // Reset to base state with proper visibility handling
            const isVisible = this.container.classList.contains('visible');
            this.container.style.transform = `translateY(${isVisible ? '0' : '100%'})`;
            this.container.style.transformOrigin = 'bottom center';
        }
        
        // Small delay to ensure DOM is completely settled
        setTimeout(() => {
            this.updatePlayerScale();
        }, 100);
    }
    ...
    updatePlayerScale() {
        // Safety check - ensure constraints are set
        if (typeof this.minScale === 'undefined' || typeof this.maxScale === 'undefined') {
            console.warn('Scaling constraints not set, skipping update');
            return;
        }
        ...
        // Use a fixed reference width for the player (typical desktop optimal width)
        const playerReferenceWidth = 1200; // Assume this is the optimal player width
        
        // Calculate scale to fit player within window width (with padding)
        const paddingBuffer = 40; // 20px padding on each side
        const availableWidth = currentWidth - paddingBuffer;
        
        // Calculate scale factor to fit the reference player width within available width
        let scaleFactor = Math.min(1.0, availableWidth / playerReferenceWidth);
        ...
        const isVisible = this.container.classList.contains('visible');
        this.container.style.transform = `translateY(${isVisible ? '0' : '100%'}) scale(${scaleFactor})`;
        this.container.style.transformOrigin = 'bottom center';
        
        // Calculate what the final player width should be
        const finalPlayerWidth = playerReferenceWidth * scaleFactor;
        
        console.log('=== RESIZE DEBUG ===');
        console.log(`Window dimensions: ${currentWidth}px x ${currentHeight}px`);
        console.log(`Player reference width: ${playerReferenceWidth}px`);
        console.log(`Available width (with padding): ${availableWidth}px`);
        console.log(`Width-based scale: ${(availableWidth / playerReferenceWidth).toFixed(3)}`);
        console.log(`Final scale factor: ${scaleFactor.toFixed(3)} (min: ${this.minScale}, max: ${this.maxScale})`);
        console.log(`Expected final player width: ${finalPlayerWidth.toFixed(1)}px (should fit in ${currentWidth}px)`);
        console.log(`Player should fit in window: ${finalPlayerWidth <= currentWidth ? 'YES' : 'NO'}`);
        console.log('==================');
    }
    ...
    showPlayer() {
        ...
        this.updatePlayerScale(); // Update scale when showing
    }
    
    hidePlayer() {
        this.container.classList.remove('visible');
        this.updatePlayerScale(); // Update scale when hiding
    }
