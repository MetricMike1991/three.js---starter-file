    ...
    updateBackgroundLogo(params) {
        ...
        textureLoader.load(params.url, (texture) => {
            ...
            const material = new THREE.SpriteMaterial({
                ...
                depthTest: false,
                depthWrite: false,
                sizeAttenuation: false // IMPORTANT: false = size is in screen pixels, not world units
            });
            ...
            // With sizeAttenuation=false, scale is relative to screen
            // Smaller values = smaller on screen
            const size = (params.size || 2) * 0.15; // Scale down for screen-space
            ...
            // Y offset moves it up/down in view
            const yOffset = params.yPosition !== undefined ? params.yPosition * 0.3 : 0.3;
            this.bgLogoSprite.position.set(0, yOffset, -2); // -Z is "in front" of camera
            console.log('[SceneManager] Sprite local position (camera space):', 0, yOffset, -2);
