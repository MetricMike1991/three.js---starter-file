...
const recordTimelineVideo = async (renderer, scene, camera, animationPlayer, mixer, options = {}) => {
    ...
    if (settings.frameWidth && settings.frameHeight && settings.containerWidth && settings.containerHeight) {
        const fovScale = settings.frameHeight / settings.containerHeight;
    ...
    const renderFrame = (timestamp) => {
        ...
        const animationTime = Math.min(elapsedSeconds, duration);
    ...
    try {
        ...
        const fileSize = formatFileSize(blob.size);
        console.log(`🎥 Video saved: ${filename} (${settings.width}×${settings.height}, ${fileSize})`);
