...
class RightMenuDropdown {
    ...
    openMenu() {
        ...
        // On desktop, close all left menus when right menu opens
...
export class RightMenuSystem {
    ...
    setupInitialState() {
        ...
        if (rightMenuContainer) {
            rightMenuContainer.classList.add('no-hover');
            // Remove no-hover class after a short delay to enable normal hover behavior
            setTimeout(() => {
                rightMenuContainer.classList.remove('no-hover');
            }, 500);
        }
    }
    ...
    setupGlobalListeners() {
        ...
        if (menuContainer) {
            ...
            menuContainer.addEventListener('mouseenter', () => {
                ...
                // Hide the left menu container when right menu becomes visible (desktop only)
                const leftMenuContainer = document.querySelector('.thumbnail-grid-container');
                if (leftMenuContainer) {
                    leftMenuContainer.classList.remove('menu-visible', 'menu-active');
        ...
        document.addEventListener('click', (e) => {
            ...
            // On mobile, don't close if click is on the left menu area
            const isMobile = window.innerWidth <= 768;
            if (isMobile) {
                const leftMenuContainer = document.querySelector('.thumbnail-grid-container');
                if (leftMenuContainer && (leftMenuContainer.contains(e.target) || e.target.closest('.menu-hint-tab'))) {
                    return; // Don't close right menu when interacting with left menu
                }
            }
            ...
            if (menuContainer && !menuContainer.contains(e.target)) {
                ...
                // On mobile, also update toggle icon and mobile-open state
                if (isMobile) {
    ...
    updateFromConfig(configTabs) {
        ...
        // Map config tab names to menu IDs
        const tabMapping = {
            'exerciseInformation': 'info1',
            'howToGuide': 'info2',
            'setupGuide': 'info3',
            'alternativeExercises': 'info4'
        };
        
        // Update each tab with config data
    ...
    toggleRightMenu() {
        ...
        if (isOpen) {
            ...
            // On desktop, close left menu first (on mobile, keep them independent)
            if (!isMobile) {
                const leftMenu = document.querySelector('.thumbnail-grid-container');
                if (leftMenu && leftMenu.classList.contains('menu-visible')) {
