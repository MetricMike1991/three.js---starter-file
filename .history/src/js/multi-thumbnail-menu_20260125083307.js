...
export class MultiThumbnailMenuSystem {
    ...
    createMobileToggleButton(menuContainer) {
        ...
        toggle.addEventListener('touchend', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('Left toggle touched!');
            self.toggleLeftMenu();
        });
        
        // Append to menu container so it slides with the menu
        menuContainer.appendChild(toggle);
        this.mobileToggleButton = toggle;
        
        // Hide the original hint tab on mobile
        const hintTab = menuContainer.querySelector('.menu-hint-tab');
        if (hintTab) {
            hintTab.style.display = 'none';
        }
        
        // Show/hide based on screen size - check immediately and after delays
    ...
    updateMobileToggleVisibility() {
        if (this.mobileToggleButton) {
            ...
            this.mobileToggleButton.style.display = isMobile ? 'flex' : 'none';
            
            // Also toggle original hint tab visibility
            const menuContainer = document.querySelector('.thumbnail-grid-container');
            const hintTab = menuContainer?.querySelector('.menu-hint-tab');
            if (hintTab) {
                hintTab.style.display = isMobile ? 'none' : '';
            }
        }
    }
