...
export class MultiThumbnailMenuSystem {
    ...
    createMobileToggleButton(menuContainer) {
        ...
        window.mobileDebug('Creating left toggle, width: ' + window.innerWidth);
        
        // Remove existing toggle if any
        const existing = document.querySelector('.mobile-menu-toggle.left-toggle');
        if (existing) existing.remove();
        
        const toggle = document.createElement('div');
        toggle.className = 'mobile-menu-toggle left-toggle';
        toggle.innerHTML = '◀';
        ...
        toggle.addEventListener('click', (e) => {
            e.stopPropagation();
            if (window.mobileDebug) window.mobileDebug('Left toggle clicked!');
            this.toggleLeftMenu();
        });
        
        // Append to viewer container or body
        const viewerContainer = document.querySelector('#flexframe-viewer-container') || document.body;
        viewerContainer.appendChild(toggle);
        this.mobileToggleButton = toggle;
        
        if (window.mobileDebug) window.mobileDebug('Left btn appended to: ' + (viewerContainer.id || 'body'));
        
        // Show/hide based on screen size - check immediately
        this.updateMobileToggleVisibility();
        // Also check after a short delay (for viewport meta to take effect)
        setTimeout(() => this.updateMobileToggleVisibility(), 100);
        setTimeout(() => this.updateMobileToggleVisibility(), 500);
    ...
    updateMobileToggleVisibility() {
        if (this.mobileToggleButton) {
            ...
            if (window.mobileDebug) window.mobileDebug('Left vis: w=' + window.innerWidth + ' mobile=' + isMobile);
            if (isMobile) {
    ...
    toggleLeftMenu() {
        const menuContainer = document.querySelector('.thumbnail-grid-container');
        if (!menuContainer) {
            if (window.mobileDebug) window.mobileDebug('ERROR: No left menu container!');
            return;
        }
        
        if (window.mobileDebug) window.mobileDebug('Menu found! Current left: ' + menuContainer.style.left);
        const isOpen = menuContainer.classList.contains('mobile-open');
        if (window.mobileDebug) window.mobileDebug('isOpen: ' + isOpen);
        ...
        if (isOpen) {
            ...
            this.updateMobileToggleIcon(false);
            if (window.mobileDebug) window.mobileDebug('Closing menu, left=-130px');
        } else {
            ...
            if (window.mobileDebug) window.mobileDebug('Opening menu, left=8px');
        }
    }
    ...
    updateMobileToggleIcon(isOpen) {
        if (this.mobileToggleButton) {
            ...
            if (window.mobileDebug) window.mobileDebug('Icon updated to: ' + (isOpen ? '✕' : '◀'));
        }
    }
}