...
export class RightMenuSystem {
    ...
    createMobileToggleButton(menuContainer) {
        ...
        hintTab.addEventListener('click', function(e) {
            if (window.innerWidth <= 768) {
                e.preventDefault();
                e.stopPropagation();
        ...
        // Make this instance accessible globally for cross-menu communication
        window.rightMenuSystem = this;
    ...
    toggleRightMenu() {
        ...
        if (isOpen) {
            ...
            if (leftMenu && leftMenu.classList.contains('mobile-open')) {
                ...
                const leftHintTab = leftMenu.querySelector('.menu-hint-tab');
                if (leftHintTab && window.innerWidth <= 768) {
                    leftHintTab.innerHTML = '▶';
                }
            }
