...
class ThumbnailDropdownMenu {
    ...
    setupSearchFilters() {
        ...
        if (!muscleFiltersContainer || !equipmentFiltersContainer) return;
    ...
    filterDataForMenu() {
        switch (this.menuType) {
            ...
            case 'search':
                ...
                // Apply muscle filters (if any selected)
                if (this.selectedMuscleFilters.size > 0) {
                    searchResults = searchResults.filter(exercise => {
                        const isPrimary = this.selectedMuscleFilters.has(exercise.information?.primaryMuscle);
                        const isSecondary = exercise.information?.secondaryMuscles?.some(m => 
                            this.selectedMuscleFilters.has(m)
                        );
                        return isPrimary || isSecondary;
                    });
                }
                
                // Apply equipment filters (if any selected)
                if (this.selectedEquipmentFilters.size > 0) {
                    searchResults = searchResults.filter(exercise => 
                        exercise.equipment.some(e => this.selectedEquipmentFilters.has(e))
                    );
                }
