...
class ThumbnailDropdownMenu {
    ...
    openMenu() {
        ...
        // On desktop, close all right menus when left menu opens
        // On mobile, keep menus independent
        if (!isMobile) {
...
export class MultiThumbnailMenuSystem {
    constructor() {
        ...
        console.log('🔒 Setting up initial no-hover state...');
        this.setupInitialState();
        console.log('✅ MultiThumbnailMenuSystem constructor complete');
    }
    
    setupInitialState() {
        // Add no-hover class to prevent menus from appearing on initial page load
        const leftMenuContainer = document.querySelector('.thumbnail-grid-container');
        if (leftMenuContainer) {
            leftMenuContainer.classList.add('no-hover');
            // Remove no-hover class after a short delay to enable normal hover behavior
            setTimeout(() => {
                leftMenuContainer.classList.remove('no-hover');
            }, 500);
        }
    }
    ...
    setupGlobalListeners() {
        ...
        if (menuContainer) {
            // Show menu on hover and keep it visible (desktop only)
            menuContainer.addEventListener('mouseenter', () => {
                const isMobile = window.innerWidth <= 768;
                if (isMobile) return; // On mobile, use click-based toggle only
                
                menuContainer.classList.add('menu-visible');
                
                // Hide the right menu container when left menu becomes visible (desktop only)
                const rightMenuContainer = document.querySelector('.thumbnail-grid-container-right');
                if (rightMenuContainer) {
                    rightMenuContainer.classList.remove('menu-visible', 'menu-active');
        ...
        document.addEventListener('click', (e) => {
            ...
            // On mobile, don't close if click is on the right menu area
            const isMobile = window.innerWidth <= 768;
            if (isMobile) {
                const rightMenuContainer = document.querySelector('.thumbnail-grid-container-right');
                if (rightMenuContainer && (rightMenuContainer.contains(e.target) || e.target.closest('.menu-hint-tab-right'))) {
                    return; // Don't close left menu when interacting with right menu
                }
            }
            ...
            if (menuContainer && !menuContainer.contains(e.target)) {
                ...
                // On mobile, also check for mobile-open state
                if (isMobile) {
    ...
    toggleLeftMenu() {
        ...
        if (isOpen) {
            ...
            // On desktop, close right menu first (on mobile, keep them independent)
            if (!isMobile) {
                const rightMenu = document.querySelector('.thumbnail-grid-container-right');
                if (rightMenu && rightMenu.classList.contains('menu-visible')) {
