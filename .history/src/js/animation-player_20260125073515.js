...
export class AnimationPlayer {
    ...
    createPlayerElements() {
        // Check if an animation player already exists (e.g., from WordPress PHP)
        const existingPlayer = document.querySelector('.animation-player');
        const existingTrigger = document.querySelector('.animation-player-trigger');
        
        if (existingPlayer) {
            // Use the existing player instead of creating a new one
            this.container = existingPlayer;
            this.container.style.display = ''; // Make sure it's visible
            
            // Create trigger if it doesn't exist
            if (existingTrigger) {
                this.triggerArea = existingTrigger;
            } else {
    ...
    initializeExistingElements() {
        // Initialize using the existing WordPress PHP-generated player
        ...
        // The existing PHP player may not have screenshot/AR buttons - add them if needed
        const playerControls = this.container.querySelector('.player-controls');
        if (playerControls && !this.screenshotBtn) {
            // Create a right section for buttons if it doesn't exist
            let playerRight = this.container.querySelector('.player-right');
            if (!playerRight) {
                playerRight = document.createElement('div');
                playerRight.className = 'player-right';
                playerControls.appendChild(playerRight);
            }
            ...
            screenshotBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20 4h-3.17L15 2H9L7.17 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 14H4V6h4.05l1.83-2h4.24l1.83 2H20v12zM12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zm0 8c-1.65 0-3-1.35-3-3s1.35-3 3-3 3 1.35 3 3-1.35 3-3 3z"/></svg>`;
            playerRight.insertBefore(screenshotBtn, playerRight.firstChild);
            ...
            arBtn.innerHTML = '<span>AR</span>';
            playerRight.insertBefore(arBtn, this.screenshotBtn.nextSibling);
        }
        ...
        // Start visible for existing player
