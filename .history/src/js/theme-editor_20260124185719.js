    ...
    setupEventListeners() {
        ...
        this.panel.querySelectorAll('.te-section-header').forEach(header => {
            header.addEventListener('click', (e) => {
                ...
                const parentSection = header.closest('.te-nested');
                
                // Check if THIS section is currently hidden
                const isHidden = window.getComputedStyle(content).display === 'none';
                
                if (isHidden) {
                    // If this is a nested section, only close other nested sections at the same level
                    if (parentSection) {
                        // Close sibling nested sections only
                        parentSection.parentElement.querySelectorAll('.te-nested > .te-section-content').forEach(otherContent => {
                            if (otherContent !== content) {
                                otherContent.style.display = 'none';
                            }
                        });
                        parentSection.parentElement.querySelectorAll('.te-nested .te-toggle-icon').forEach(otherIcon => {
                            if (otherIcon !== icon) {
                        ...
                        this.panel.querySelectorAll('.te-panel > .te-content > .te-section > .te-section-content').forEach(otherContent => {
                            otherContent.style.display = 'none';
                        });
                        // Reset all top-level icons
                        this.panel.querySelectorAll('.te-panel > .te-content > .te-section > .te-section-header .te-toggle-icon').forEach(otherIcon => {
                            otherIcon.textContent = '▶';
                        });
                    }
