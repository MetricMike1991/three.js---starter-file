...
class ThemeEditor {
    ...
    updatePlayerStyling() {
        ...
        // Buttons - target all button types in the player
        const btnColor = this.currentSettings.playerButtonColor;
        const btnOpacity = this.currentSettings.playerButtonOpacity;
        const buttonSelectors = '.play-pause-btn, .screenshot-btn, .ar-btn, .quality-btn, .speed-btn, .control-btn';
        player.querySelectorAll(buttonSelectors).forEach(btn => {
            btn.style.backgroundColor = this.hexToRgba(btnColor, btnOpacity);
        });
        
        // Icons - target SVGs and text in buttons
        const iconColor = this.currentSettings.playerIconColor;
        player.querySelectorAll(`${buttonSelectors.split(',').map(s => s.trim() + ' svg').join(', ')}`).forEach(svg => {
            svg.style.fill = iconColor;
        });
        // Also color button text
        player.querySelectorAll('.speed-btn span, .ar-btn span, .quality-btn span').forEach(span => {
            span.style.color = iconColor;
        });
        // Time display
        player.querySelectorAll('.time-display, .time-display span').forEach(el => {
            el.style.color = iconColor;
        });
        
        // Accent color (timeline slider thumb)
        const accentColor = this.currentSettings.playerAccentColor;
        const slider = player.querySelector('.timeline-slider');
        if (slider) {
            // Inject dynamic CSS for the slider thumb (can't style pseudo-elements directly)
            let styleEl = document.getElementById('te-player-accent-style');
            if (!styleEl) {
                styleEl = document.createElement('style');
                styleEl.id = 'te-player-accent-style';
                document.head.appendChild(styleEl);
            }
            styleEl.textContent = `
                .timeline-slider::-webkit-slider-thumb { background: ${accentColor} !important; }
                .timeline-slider::-moz-range-thumb { background: ${accentColor} !important; }
            `;
        }
    }
    ...
    updateMenuStyling() {
        ...
        // Update left thumbnail menu container
        const leftMenu = document.querySelector('.thumbnail-grid-container');
        if (leftMenu) {
            leftMenu.style.backgroundColor = bgRgba;
        }
        
        // Update left menu dropdowns
        document.querySelectorAll('.thumbnail-dropdown').forEach(dropdown => {
            dropdown.style.backgroundColor = bgRgba;
        });
        
        // Update left menu toggle buttons
        document.querySelectorAll('.thumbnail-menu-toggle').forEach(btn => {
            btn.style.backgroundColor = bgRgba;
            btn.style.color = textColor;
        });
        
        // Update thumbnail items text
        document.querySelectorAll('.thumbnail-item, .thumbnail-label').forEach(el => {
            el.style.color = textColor;
        });
        
        // Update right menu container
        const rightMenu = document.querySelector('.thumbnail-grid-container-right');
        if (rightMenu) {
            rightMenu.style.backgroundColor = bgRgba;
        }
        
        // Update right menu dropdowns
        document.querySelectorAll('.thumbnail-dropdown-right').forEach(dropdown => {
            dropdown.style.backgroundColor = bgRgba;
        });
        
        // Update right menu info items
        document.querySelectorAll('.info-step-item, .info-step-title, .info-step-text').forEach(el => {
            el.style.color = textColor;
        });
        
        // Update scroll buttons in both menus
        document.querySelectorAll('.scroll-btn').forEach(btn => {
            btn.style.backgroundColor = this.hexToRgba(bgColor, Math.min(bgOpacity + 0.2, 1));
            btn.style.color = textColor;
        });
        
        // Inject accent color styles for hover states
        let styleEl = document.getElementById('te-menu-accent-style');
        if (!styleEl) {
            styleEl = document.createElement('style');
            styleEl.id = 'te-menu-accent-style';
            document.head.appendChild(styleEl);
        }
        styleEl.textContent = `
