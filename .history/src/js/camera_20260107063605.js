...
class CameraManager {
    constructor(canvas, sizes) {
        ...
        this.zoomDecay = 0.92;
        this.zoomMomentumThreshold = 0.01;
    ...
    init() {
        ...
        this.controls.minDistance = 0.01;  // Much closer zoom
        this.controls.maxDistance = 200;   // Much farther zoom
    ...
    setupEventListeners() {
        ...
        // Spacebar to reset rotation center
    ...
    resetRotationCenter() {
        console.log('Resetting rotation center to origin');
        
        // Immediate controls damping adjustment for faster response
        const originalDamping = this.controls.dampingFactor;
        this.controls.dampingFactor = 0.15;
        
        // Start smooth lerp back to original target
        this.targetLerpFrom.copy(this.controls.target);
    ...
    trackZoomMomentum(event) {
        ...
        this.zoomVelocity = scrollDelta * 0.02 * multiplier;
    ...
    updateZoomMomentum() {
        if (Math.abs(this.zoomMomentum) > this.zoomMomentumThreshold) {
            ...
            const momentumZoom = this.zoomMomentum * distance * 0.1;
            ...
            const newDistance = Math.max(
                ...
                Math.min(this.controls.maxDistance, distance + momentumZoom)
            );
