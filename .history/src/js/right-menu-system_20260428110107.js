...
class RightMenuDropdown {
    ...
    renderSuggestedExercises(exerciseIds) {
        ...
        foundExercises.forEach(exercise => {
            ...
            // Thumbnail image (with fallback) - add default-thumbnail class if not a custom upload
            const thumbUrl = exercise.thumbnailUrl || '';
            const thumbClass = exercise.hasCustomThumbnail ? '' : 'default-thumbnail';
            const imgHTML = thumbUrl 
                ? `<img class="${thumbClass}" src="${thumbUrl}" alt="${exercise.name}" loading="lazy">`
            ...
            card.addEventListener('click', (e) => {
                e.stopPropagation();
                // Prevent rapid double-clicks that would queue up multiple model loads
                if (RightMenuSystem._suggestedClickLock) return;
                RightMenuSystem._suggestedClickLock = true;
                // Visually disable all suggested cards until cooldown elapses
                document.querySelectorAll('.info-suggested-card').forEach(c => {
                    c.classList.add('is-loading');
                    c.style.pointerEvents = 'none';
                });
                setTimeout(() => {
                    RightMenuSystem._suggestedClickLock = false;
                    document.querySelectorAll('.info-suggested-card').forEach(c => {
