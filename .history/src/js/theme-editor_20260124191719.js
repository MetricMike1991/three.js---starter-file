            ...
            header.addEventListener('click', (e) => {
                ...
                if (isHidden) {
                    if (isNested) {
                        ...
                        // Show actual UI elements when their sections are open
                        if (section === 'ui-player') {
                            // Show animation player (bottom menu)
                        ...
                        } else if (section === 'ui-menu') {
                            // Show side menus - try multiple selectors
                            const menus = document.querySelectorAll('.thumbnail-menu-wrapper, .thumbnail-menu-toggle, [class*="menu"]');
                            menus.forEach(menu => {
                                menu.style.visibility = 'visible';
                                menu.style.opacity = '1';
                                menu.style.display = '';
                            });
                            // Also try showing dropdowns
                    ...
                    // Hide UI elements when their sections are closed (if nested)
                    if (isNested) {
                        if (section === 'ui-player') {
                            const player = document.querySelector('#animation-player');
                            if (player && !player.classList.contains('always-visible')) {
