...
const recordTimelineVideo = async (renderer, scene, camera, animationPlayer, mixer, options = {}) => {
    ...
    // Logo plane — positioned by overlayLogoPosition
    if (settings.overlayLogoUrl) {
        ...
        if (img) {
            ...
            const logoPos = settings.overlayLogoPosition || 'top-left';
            const x = logoPos === 'top-center' ? 0 : -vidAspect + pad + logoW / 2;
            mesh.position.set(x, 1 - pad - logoH / 2, -1);
            hudScene.add(mesh);
