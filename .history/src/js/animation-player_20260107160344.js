...
export class AnimationPlayer {
    ...
    updatePlayerScale() {
        ...
        // Get player container dimensions BEFORE scaling
        this.container.style.transform = 'translateY(100%) scale(1)'; // Reset to measure natural size
        const containerRect = this.container.getBoundingClientRect();
        const playerNaturalWidth = containerRect.width;
        
        // Calculate scale to fit player within window width (with some padding)
        const paddingBuffer = 40; // 20px padding on each side
        const availableWidth = currentWidth - paddingBuffer;
        
        // Calculate scale factor to fit the player within available width
        let scaleFactor = Math.min(1.0, availableWidth / playerNaturalWidth);
        
        // Also apply the original screen-size based scaling (but cap it)
        ...
        this.container.style.transform = `translateY(${this.container.classList.contains('visible') ? '0' : '100%'}) scale(${scaleFactor})`;
        ...
        console.log(`Player natural width: ${playerNaturalWidth.toFixed(1)}px`);
        console.log(`Available width (with padding): ${availableWidth}px`);
        console.log(`Width-based scale: ${(availableWidth / playerNaturalWidth).toFixed(3)}`);
        ...
        console.log(`Final player width: ${finalRect.width.toFixed(1)}px (should fit in ${currentWidth}px)`);
        console.log(`Player fits in window: ${finalRect.width <= currentWidth ? 'YES' : 'NO'}`);
        console.log('==================');
    }
