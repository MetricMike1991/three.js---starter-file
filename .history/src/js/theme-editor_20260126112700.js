...
class ThemeEditor {
    ...
    updateMenuStyling() {
        ...
        styleEl.textContent = `
            ...
            .thumbnail-item:hover {
                border-color: ${primaryColor} !important;
                box-shadow: 0 0 15px ${this.hexToRgba(primaryColor, 0.5)}, 0 5px 20px rgba(0, 0, 0, 0.3) !important;
            }
            .thumbnail-label {
    ...
    async saveTheme() {
        ...
        try {
            ...
            if (window.flexframeSettings && window.flexframeSettings.ajaxUrl) {
                ...
                if (result.success) {
                    this.showMessage(`Theme "${themeName}" saved! Settings copied to clipboard.`, 'success');
                    this.panel.querySelector('#te-theme-name').value = '';
                    // Copy settings to clipboard
                    this.copySettingsToClipboard(themeName, presetData);
                } else {
                ...
                this.showMessage(`Theme "${themeName}" saved locally! Settings copied to clipboard.`, 'success');
                this.panel.querySelector('#te-theme-name').value = '';
                // Copy settings to clipboard
                this.copySettingsToClipboard(themeName, presetData);
            }
    ...
    copySettingsToClipboard(themeName, presetData) {
        // Create a formatted JSON string with the theme settings
        const clipboardData = {
            themeName: themeName,
        ...
        navigator.clipboard.writeText(jsonString).then(() => {
            ...
            console.error('[Theme Editor] Failed to copy to clipboard:', err);
            ...
            console.log('[Theme Editor] Settings copied to clipboard (fallback)');
        });
    }
