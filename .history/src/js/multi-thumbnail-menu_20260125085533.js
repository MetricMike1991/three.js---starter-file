...
class ThumbnailDropdownMenu {
    ...
    constructor(menuType) {
        ...
        this.isOpen = false;
        this.hasBeenOpened = false; // Track if dropdown has been opened before
    ...
    filterDataForMenu() {
        switch (this.menuType) {
            case 'exercises':
                ...
                // If both muscle and equipment are selected, apply both filters
                if (selectedMuscle && selectedEquipment) {
                    this.filteredData = this.allExercises.filter(exercise => {
                        // Check muscle match (primary or secondary)
                        ...
                        const muscleMatch = isPrimary || isSecondary;
                        
                        // Check equipment match
                        const equipmentMatch = exercise.equipment.includes(selectedEquipment);
                    ...
                    console.log(`Filtering exercises by muscle: ${selectedMuscle} AND equipment: ${selectedEquipment}, found ${this.filteredData.length} exercises`);
                }
                // Filter by selected muscle only
                ...
                // Filter by selected equipment only
                else if (selectedEquipment) {
                    this.filteredData = this.allExercises.filter(exercise => 
                        exercise.equipment.includes(selectedEquipment)
                    );
                    console.log(`Filtering exercises by equipment: ${selectedEquipment}, found ${this.filteredData.length} exercises`);
                }
...
export class MultiThumbnailMenuSystem {
    ...
    createMobileToggleButton(menuContainer) {
        ...
        this.createMobileHamburgerMenu();
    }

    createMobileHamburgerMenu() {
        // Remove existing hamburger if any
        const existing = document.querySelector('.mobile-hamburger-menu');
        ...
        // Section: Exercises (left menu buttons)
        const exerciseSection = this.createMenuSection('Select View', [
            { icon: '🏋️', label: 'Exercises', action: () => this.triggerMenuButton('exercises') },
            { icon: '💪', label: 'Muscles', action: () => this.triggerMenuButton('muscles') },
            { icon: '🔧', label: 'Equipment', action: () => this.triggerMenuButton('equipment') },
            { icon: '🔍', label: 'Search', action: () => this.triggerMenuButton('search') }
        ]);
        menuContent.appendChild(exerciseSection);
        
        // Section: Info (right menu buttons)
        const infoSection = this.createMenuSection('Information', [
            { icon: 'ℹ️', label: 'Exercise Info', action: () => this.triggerRightMenuButton('info1') },
            { icon: '📖', label: 'How To Guide', action: () => this.triggerRightMenuButton('info2') },
            { icon: '⚙️', label: 'Setup Guide', action: () => this.triggerRightMenuButton('info3') },
            { icon: '🔄', label: 'Alternatives', action: () => this.triggerRightMenuButton('info4') }
        ]);
        menuContent.appendChild(infoSection);
        
        overlay.appendChild(menuContent);
        
        // Add to DOM
        const viewerContainer = document.querySelector('#flexframe-viewer-container') || document.body;
        viewerContainer.appendChild(hamburger);
    ...
    triggerMenuButton(menuType) {
        // Find and click the corresponding menu button
        const menuContainer = document.querySelector('.thumbnail-grid-container');
        if (!menuContainer) return;
        
        const buttons = menuContainer.querySelectorAll('.thumbnail-menu-toggle');
        const typeMap = { 'exercises': 0, 'muscles': 1, 'equipment': 2, 'search': 3 };
        const index = typeMap[menuType];
    ...
    triggerRightMenuButton(menuId) {
        // Find and click the corresponding right menu button
        const rightContainer = document.querySelector('.thumbnail-grid-container-right');
        if (!rightContainer) return;
        
        const idMap = { 'info1': 0, 'info2': 1, 'info3': 2, 'info4': 3 };
        const buttons = rightContainer.querySelectorAll('.thumbnail-menu-toggle');
        const index = idMap[menuId];
    ...
    toggleMobileMenu() {
        if (this.menuOverlay.style.display === 'none' || !this.menuOverlay.style.display) {
            this.openMobileMenu();
        } else {
            this.closeMobileMenu();
        }
    ...
    updateMobileToggleVisibility() {
        ...
        if (isMobile && !this.hamburgerBtn) {
            this.createMobileHamburgerMenu();
        }
        
        if (this.hamburgerBtn) {
            this.hamburgerBtn.style.display = isMobile ? 'flex' : 'none';
        }
        if (this.menuOverlay && !isMobile) {
            this.menuOverlay.style.display = 'none';
        }
        
        // Hide/show the original menus
        ...
        if (leftMenu) {
            leftMenu.style.display = isMobile ? 'none' : '';
        }
        if (rightMenu) {
            rightMenu.style.display = isMobile ? 'none' : '';
        }
    }

    toggleLeftMenu() {
        // Legacy - not used with hamburger menu
    }
