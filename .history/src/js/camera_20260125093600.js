...
class CameraManager {
    ...
    init() {
        ...
        // Zoom out more on mobile devices
        const isMobile = window.innerWidth <= 768;
        const mobileZoomFactor = 1.5; // How much further to position camera on mobile
    ...
    // Update the original camera state for reset functionality
    updateOriginalState(position, rotation, target) {
        if (position) {
            this.originalPosition.set(...position);
        }
        if (rotation) {
            this.originalRotation.x = rotation[0];
            this.originalRotation.y = rotation[1];
            this.originalRotation.z = rotation[2];
        }
        if (target) {
            this.originalTarget.set(...target);
        }
        console.log('Updated original camera state for spacebar reset');
    }
