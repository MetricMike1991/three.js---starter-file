...
class ThemeEditor {
    ...
    setupEventListeners() {
        ...
        // Section toggles - accordion behavior (only one open at a time)
        this.panel.querySelectorAll('.te-section-header').forEach(header => {
            header.addEventListener('click', () => {
                ...
                if (isHidden) {
                    ...
                    this.panel.querySelectorAll('.te-section-content').forEach(otherContent => {
                        if (otherContent !== content) {
                            otherContent.style.display = 'none';
                            // Find and update the other section's icon
                            const otherId = otherContent.id.replace('section-', '');
                            const otherHeader = this.panel.querySelector(`.te-section-header[data-section="${otherId}"]`);
                            if (otherHeader) {
                                const otherIcon = otherHeader.querySelector('.te-toggle-icon');
