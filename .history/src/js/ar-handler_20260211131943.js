...
export class ARHandler {
    ...
    launchAndroidAR() {
        ...
        const cleanGlbUrl = decodeURIComponent(glbUrl);

        // Build Scene Viewer URL with branding parameters
        let sceneViewerParams = [
            `file=${encodeURIComponent(cleanGlbUrl)}`,
            `mode=ar_only`,
        ...
        // Use Google's Scene Viewer intent via ARCore package
        const intentUrl = `intent://arvr.google.com/scene-viewer/1.0?` +
            sceneViewerParams.join('&') +
            `#Intent;scheme=https;package=com.google.ar.core;` +
            `action=android.intent.action.VIEW;` +
            `S.browser_fallback_url=${encodeURIComponent(cleanGlbUrl)};end;`;
