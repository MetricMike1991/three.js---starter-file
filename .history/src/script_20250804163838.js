...
const metcapTexture = textureLoader.load('static/textures/matcaps/1.png')
...
// Zoom limits
const minDistance = 0.5  // Closest zoom (prevents clipping)
const maxDistance = 50   // Furthest zoom
...
const tick = () =>
{
    ...
    if (Math.abs(zoomVelocity) > 0.001) {
        ...
        // Check zoom limits before applying movement
        const newDistance = currentDistance + zoomVelocity
        
        if (newDistance >= minDistance && newDistance <= maxDistance) {
            // Safe to zoom - apply full velocity
            camera.position.addScaledVector(direction, zoomVelocity)
        } else if (newDistance < minDistance) {
            // Too close - only allow zooming out
            if (zoomVelocity < 0) { // Zooming out (moving away)
                camera.position.addScaledVector(direction, zoomVelocity)
            }
            // Stop velocity if trying to zoom in too close
            if (zoomVelocity > 0) {
                zoomVelocity = 0
            }
        } else if (newDistance > maxDistance) {
            // Too far - only allow zooming in
            if (zoomVelocity > 0) { // Zooming in (moving closer)
                camera.position.addScaledVector(direction, zoomVelocity)
            }
            // Stop velocity if trying to zoom out too far
            if (zoomVelocity < 0) {
                zoomVelocity = 0
            }
        }
