...
// Override touch panning with custom sensitivity
canvas.addEventListener('touchstart', (event) => {
    if (event.touches.length === 2) {
        ...
        // Temporarily increase pan speed for touch
        controls.panSpeed = touchPanMultiplier
    }
})

canvas.addEventListener('touchend', (event) => {
    // Reset pan speed when touch ends
...
canvas.addEventListener('touchmove', (event) => {
    if (event.touches.length === 2) {
        ...
        if (lastTouchDistance > 0) {
            ...
            // 70x speed for touch zoom (10x the previous 7x)
            zoomVelocity += delta * zoomSensitivity * 250
            zoomVelocity = Math.max(-0.5, Math.min(0.5, zoomVelocity))
        }
        
        lastTouchDistance = currentDistance
        
        // Ensure touch pan speed is set for OrbitControls
        controls.panSpeed = touchPanMultiplier
    }
})
