...
class ThreeJSApp {
    ...
    updateScreenshotFramePanel(width, height) {
        ...
        // Center the frame on the canvas area (offset from container origin)
        const offsetX = refRect.left - containerRect.left;
        const offsetY = refRect.top - containerRect.top;
        const left = offsetX + (containerWidth - frameWidth) / 2;
        const top = offsetY + (containerHeight - frameHeight) / 2;
