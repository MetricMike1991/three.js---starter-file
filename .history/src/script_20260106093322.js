...
async function importSettingsFromClipboard() {
    try {
        ...
        // Dust particles
        if (settings.dustParticles) {
            Object.assign(dustParams, settings.dustParticles);
            createDustParticles();
        }
        // Light
        if (settings.light) {
...
// Add missing solid ground material
...
function createDustParticles() {
    ...
    dustPositions = new Float32Array(dustParams.count * 3);
    dustVelocities = new Float32Array(dustParams.count * 3);
    dustSizes = new Float32Array(dustParams.count);
    ...
    for (let i = 0; i < dustParams.count; i++) {
        ...
        dustPositions[i3] = (Math.random() - 0.5) * dustParams.horizontalRange * 2;
        dustPositions[i3 + 1] = Math.random() * dustParams.verticalRange + dustParams.verticalOffset;
        dustPositions[i3 + 2] = (Math.random() - 0.5) * dustParams.horizontalRange * 2;
        
        // Random velocities (very slow floating motion)
        dustVelocities[i3] = (Math.random() - 0.5) * 0.001;
        dustVelocities[i3 + 1] = (Math.random() - 0.5) * 0.0005;
        dustVelocities[i3 + 2] = (Math.random() - 0.5) * 0.001;
        
        // Random sizes
        dustSizes[i] = dustParams.size * (1 + (Math.random() - 0.5) * dustParams.sizeRandomness);
    }
    ...
    dustMaterial = new THREE.PointsMaterial({
        color: dustParams.color,
        size: dustParams.size,
        transparent: true,
        opacity: dustParams.opacity,
    ...
    dustParticles.visible = dustParams.visible;
    scene.add(dustParticles);
}

// Function to update dust particles
function updateDustParticles(deltaTime) {
    if (!dustParticles || !dustParams.visible) return;
    ...
    for (let i = 0; i < dustParams.count; i++) {
        ...
        positions[i3] += dustVelocities[i3] * dustParams.speed * deltaTime * 1000;
        positions[i3 + 1] += dustVelocities[i3 + 1] * dustParams.speed * deltaTime * 1000;
        positions[i3 + 2] += dustVelocities[i3 + 2] * dustParams.speed * deltaTime * 1000;
        
        // Wrap around boundaries
        if (positions[i3] > dustParams.horizontalRange) positions[i3] = -dustParams.horizontalRange;
        if (positions[i3] < -dustParams.horizontalRange) positions[i3] = dustParams.horizontalRange;
        if (positions[i3 + 2] > dustParams.horizontalRange) positions[i3 + 2] = -dustParams.horizontalRange;
        if (positions[i3 + 2] < -dustParams.horizontalRange) positions[i3 + 2] = dustParams.horizontalRange;
        
        // Reset particles that fall too low or go too high
        if (positions[i3 + 1] < dustParams.verticalOffset - 0.5 || 
            positions[i3 + 1] > dustParams.verticalOffset + dustParams.verticalRange + 0.5) {
            positions[i3 + 1] = Math.random() * dustParams.verticalRange + dustParams.verticalOffset;
        }
    }
