...
// ---------------------------------------------
// 9. Default scene settings (move up for early use)
// ---------------------------------------------
...
// ---------------------------------------------
// 11. Renderer: WebGLRenderer setup (move up for early use)
// ---------------------------------------------
...
// Set camera position and rotation from defaultSettings
camera.position.set(
...
// ---------------------------------------------
// 14. Animation Loop: Updates and renders the scene
// ---------------------------------------------
const tick = () => {
...
// Add a gradient alpha map for ground fade effect
const fadeTexture = textureLoader.load('./textures/gradients/3.jpg');
groundMaterial.alphaMap = fadeTexture;
groundMaterial.transparent = true;
groundMaterial.alphaMap.wrapS = THREE.ClampToEdgeWrapping;
groundMaterial.alphaMap.wrapT = THREE.ClampToEdgeWrapping;
...
// Add Copy Camera/Target to Clipboard button to GUI
const copyCameraToClipboard = () => {
    ...
    const camStr = JSON.stringify(camSettings, null, 2);
    navigator.clipboard.writeText(camStr).then(() => {
        alert('Camera & target copied to clipboard!');
    }, () => {
        alert('Failed to copy camera settings to clipboard.');
    });
};

gui.add({ copyCameraToClipboard }, 'copyCameraToClipboard').name('Copy Camera/Target to Clipboard');
...
// Set controls target to the center of 'Cube187_METAL_0' on default load
let setInitialTarget = () => {
    let found = null;
    scene.traverse(obj => {
        if (obj.isMesh && obj.name === 'Cube187_METAL_0') found = obj;
    });
    if (found) {
        // Use the mesh's world position as the target
        found.updateMatrixWorld();
...
// Wait until the model is loaded before setting the target
if (allClickableMeshes.length > 0) {
    setInitialTarget();
} else {
    // If model loads async, hook into GLTFLoader callback
    const origOnLoad = gltfLoader.manager.onLoad;
