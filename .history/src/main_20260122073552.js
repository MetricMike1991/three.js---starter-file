...
class ThreeJSApp {
    ...
    applyWordPressSceneSettings() {
        ...
        // Apply Background Logo Watermark from WordPress
        if (window.flexframeSettings?.bgLogoSettings && this.sceneManager && this.cameraManager) {
            const logoSettings = window.flexframeSettings.bgLogoSettings;
            console.log('[FlexFrame Scene] Applying WordPress background logo settings:', logoSettings);
            
            this.sceneManager.updateBackgroundLogo({
                enabled: logoSettings.enabled || false,
                url: logoSettings.url || '',
                size: logoSettings.size || 2,
                opacity: logoSettings.opacity || 0.15,
                yPosition: 1.5, // Upper area of view
                camera: this.cameraManager.camera // Pass camera reference
            });
        }
