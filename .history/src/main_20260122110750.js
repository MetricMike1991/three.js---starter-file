...
class ThreeJSApp {
    ...
    createScreenshotPanel() {
        ...
        panel.innerHTML = `
            ...
            <div class="screenshot-panel-content">
                ...
                <div class="screenshot-row checkbox-row">
                    <label>Show Frame Preview</label>
                    <input type="checkbox" id="ss-show-frame">
                </div>
                <div class="screenshot-row">
                ...
                <div class="screenshot-buttons">
                    <button class="ss-btn ss-quick">📷 Quick Shot</button>
                    <button class="ss-btn ss-custom">📸 Custom Shot</button>
                </div>
            </div>
        `;
        ...
        // Show frame preview checkbox
        panel.querySelector('#ss-show-frame').addEventListener('change', (e) => {
            const width = parseInt(panel.querySelector('#ss-width').value);
            const height = parseInt(panel.querySelector('#ss-height').value);
            this.toggleScreenshotFrame(e.target.checked);
            if (e.target.checked) {
                this.updateScreenshotFrame(width, height);
            }
        });
        
        // Update frame when dimensions change
        panel.querySelector('#ss-width').addEventListener('input', (e) => {
            if (panel.querySelector('#ss-show-frame').checked) {
                this.updateScreenshotFrame(parseInt(e.target.value), parseInt(panel.querySelector('#ss-height').value));
            }
        });
        panel.querySelector('#ss-height').addEventListener('input', (e) => {
            if (panel.querySelector('#ss-show-frame').checked) {
                this.updateScreenshotFrame(parseInt(panel.querySelector('#ss-width').value), parseInt(e.target.value));
            }
        });
        
        // Quick shot button
        panel.querySelector('.ss-quick').addEventListener('click', () => {
            this.takeQuickScreenshot();
        });
        ...
        document.addEventListener('exercisesSelected', () => {
            if (this.currentExerciseName) {
                panel.querySelector('#ss-filename').value = `${this.currentExerciseName} Thumbnail`;
            }
        });
    ...
    toggleScreenshotPanel(forceState) {
        ...
        // Update filename when opening
        if (newState && this.currentExerciseName) {
            this.screenshotPanel.querySelector('#ss-filename').value = `${this.currentExerciseName} Thumbnail`;
        }
    }
    ...
    async takeCustomScreenshot() {
        ...
        const filename = this.screenshotPanel?.querySelector('#ss-filename')?.value || 'screenshot';
        const format = this.screenshotPanel?.querySelector('#ss-format')?.value || 'png';
        const transparent = this.screenshotPanel?.querySelector('#ss-transparent')?.checked || false;
