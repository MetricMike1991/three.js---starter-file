...
// sharedMaterial.displacementScale = .1
...
// sharedMaterial.normalScale.set(0.5, 0.5) // Adjust normal map
...
// sharedMaterial.clearcoat = 1.0
// sharedMaterial.clearcoatRoughness = 0

// gui.add(sharedMaterial, 'clearcoat').min(0).max(1).step(0.0001)
// gui.add(sharedMaterial, 'clearcoatRoughness').min(0).max(1).step(0.0001)

//sheen properties
// sharedMaterial.sheen = 1.0
// sharedMaterial.sheenColor.set(0x000000) // Default sheen color
// sharedMaterial.sheenRoughness = 0.5

// gui.add(sharedMaterial, 'sheen').min(0).max(1).step(0.0001)
// gui.add(sharedMaterial, 'sheenRoughness').min(0).max(1).step(0.0001)
// gui.addColor(sharedMaterial, 'sheenColor')

// //Iridescence properties
// sharedMaterial.iridescence = 1.0 // Full iridescence effect
// sharedMaterial.iridescenceIOR = 1.3 // Index of refraction for iridescence
// sharedMaterial.iridescenceThicknessRange = [100, 800] // Range of thickness for iridescence

// gui.add(sharedMaterial, 'iridescence').min(0).max(1).step(0.0001)
// gui.add(sharedMaterial, 'iridescenceIOR').min(1).max(2.333).step(0.0001)

// // Create a helper object to control thickness range
// const iridescenceControls = {
//     thicknessMin: sharedMaterial.iridescenceThicknessRange[0],
//     thicknessMax: sharedMaterial.iridescenceThicknessRange[1]
// }

// gui.add(iridescenceControls, 'thicknessMin').min(0).max(1000).step(1).onChange((value) => {
//     sharedMaterial.iridescenceThicknessRange[0] = value
// })
// gui.add(iridescenceControls, 'thicknessMax').min(0).max(1000).step(1).onChange((value) => {
//     sharedMaterial.iridescenceThicknessRange[1] = value
// })


//Transmission properties
sharedMaterial.transmission = 1.0 // Full transmission effect
sharedMaterial.thickness = 1.0 // Thickness of the material for transmission
sharedMaterial.ior = 1.5 // Index of refraction for transmission

gui.add(sharedMaterial, 'transmission').min(0).max(1).step(0.0001)
gui.add(sharedMaterial, 'thickness').min(0).max(10).step(0.01)
gui.add(sharedMaterial, 'ior').min(1).max(2.333).step(0.001)
...
const geometry3 = new THREE.TorusGeometry(0.5, 0.2, 64, 128)
