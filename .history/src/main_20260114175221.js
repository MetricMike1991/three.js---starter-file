                                ...
                                if (mat.name) {
                                    ...
                                    // Convert SKIN materials to MeshPhysicalMaterial for advanced transparency
                                        ...
                                        if (convertedMaterials.has(mat.name)) {
                                            ...
                                            // Create new MeshPhysicalMaterial with Sketchfab-style refraction settings
                                            const physicalMat = new THREE.MeshPhysicalMaterial({
                                                color: mat.color || new THREE.Color(0xffffff),
                                                map: mat.map,
                                                normalMap: mat.normalMap,
                                                roughness: 0, // Sketchfab uses 0 for glass-like surface
                                                metalness: 0,
                                                emissive: mat.emissive || new THREE.Color(0x000000),
                                                emissiveIntensity: mat.emissiveIntensity || 0,
                                                emissiveMap: mat.emissiveMap,
                                                opacity: 1.0, // Full opacity when using transmission
                                                transparent: true,
                                                side: THREE.DoubleSide,
                                                depthWrite: false,
                                                depthTest: true,
                                                blending: THREE.NormalBlending, // Normal blending for refraction mode
                                                // Refraction/transmission properties (Sketchfab "Refraction" blend mode)
                                                transmission: 0.721, // This creates the refraction effect (like Sketchfab's refraction mode)
                                                thickness: 1.0,
                                                ior: 1.0, // Sketchfab IOR value
