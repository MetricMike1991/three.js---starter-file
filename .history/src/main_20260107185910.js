...
import { MultiMenuSystem } from './js/multi-menu.js';
...
class ThreeJSApp {
    ...
    async init() {
        ...
        // Initialize multi-menu system
        this.multiMenuSystem = new MultiMenuSystem();
        
        // Listen for menu selection events
        document.addEventListener('exerciseSelected', (e) => {
            console.log('Exercise selected:', e.detail.item);
        });
        
        document.addEventListener('musclesSelected', (e) => {
            console.log('Muscles selected:', e.detail.item);
        });
        
        document.addEventListener('equipmentSelected', (e) => {
            console.log('Equipment selected:', e.detail.item);
        });
        
        document.addEventListener('searchSelected', (e) => {
            console.log('Search result selected:', e.detail.item);
        });
        ...
        this.settingsManager.registerManager('multiMenuSystem', this.multiMenuSystem);
    ...
    setupGUI() {
        ...
        // Multi-menu system controls
        this.setupMultiMenuGUI();
    ...
    setupMultiMenuGUI() {
        const menuFolder = this.trackFolder(this.gui.addFolder('🖼️ Multi Menu System'));
        const settings = this.multiMenuSystem.settings;
        
        // Width percentage control (kept for consistency but not used in new system)
        menuFolder.add(settings, 'widthPercentage', 20, 100, 5)
            .name('Width %')
            .onChange(value => {
                this.multiMenuSystem.updateAllSettings({ widthPercentage: value });
                console.log('Multi-menu width:', value + '%');
            });
        
        // Background color
        menuFolder.addColor(settings, 'backgroundColor')
            .name('Background Color')
            .onChange(value => {
                this.multiMenuSystem.updateAllSettings({ backgroundColor: value });
                console.log('Multi-menu background color:', value);
            });
        
        // Background opacity
        menuFolder.add(settings, 'backgroundOpacity', 0, 1, 0.1)
            .name('Background Opacity')
            .onChange(value => {
                this.multiMenuSystem.updateAllSettings({ backgroundOpacity: value });
                console.log('Multi-menu opacity:', value);
            });
        
        // Border radius
        menuFolder.add(settings, 'borderRadius', 0, 30, 1)
            .name('Container Radius')
            .onChange(value => {
                this.multiMenuSystem.updateAllSettings({ borderRadius: value });
                console.log('Multi-menu container radius:', value + 'px');
            });
        
        // Thumbnail border radius
        menuFolder.add(settings, 'thumbnailRadius', 0, 20, 1)
            .name('Item Radius')
            .onChange(value => {
                this.multiMenuSystem.updateAllSettings({ thumbnailRadius: value });
                console.log('Multi-menu item radius:', value + 'px');
            });
        
        // Keep menu open toggle
        menuFolder.add(settings, 'keepOpen')
            .name('Keep Menus Open')
            .onChange(value => {
                this.multiMenuSystem.updateAllSettings({ keepOpen: value });
                console.log('Multi-menu keep open:', value ? 'ON' : 'OFF');
            });
        
        // Copy settings button
        menuFolder.add({
            copySettings: () => this.multiMenuSystem.copySettingsToClipboard()
