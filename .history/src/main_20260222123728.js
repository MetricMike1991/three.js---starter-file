...
class ThreeJSApp {
            ...
            if (exercise.source === 'custom' && exercise.youtubeUrl) {
                ...
                if (exercise.showInfo && exercise.information && window.rightMenuManager) {
                    ...
                    const rightMenuTabs = {
                        exerciseInformation: {
                            title: exercise.name,
                            sections: sections.length ? sections : [{ heading: exercise.name, content: 'Custom exercise' }]
                        },
    ...
    showYouTubeViewer(youtubeUrl, exerciseId, exerciseName) {
        ...
        // Store for capture feature
        this._ytVideoId = videoId;
        this._ytExerciseId = exerciseId || '';
        this._ytExerciseName = exerciseName || '';
        ...
        // Check if admin and not mobile (show capture button)
        const ws = window.flexframeSettings || {};
        const isAdmin = ws.isAdmin;
        const isMobile = window.innerWidth <= 768;
        const captureBtn = (isAdmin && !isMobile)
            ? `<button id="ffx-yt-capture-btn" title="Capture thumbnail from this video">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 15.2a3.2 3.2 0 1 0 0-6.4 3.2 3.2 0 0 0 0 6.4z"/><path d="M9 2L7.17 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2h-3.17L15 2H9zm3 15c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5z"/></svg>
        ...
        overlay.innerHTML = `
            <div class="ffx-yt-wrapper">
                ...
                ${captureBtn ? `<div class="ffx-yt-capture-wrap">${captureBtn}</div>` : ''}
            </div>`;
        
        // Bind capture button
        const captureBtnEl = overlay.querySelector('#ffx-yt-capture-btn');
        if (captureBtnEl) {
            captureBtnEl.addEventListener('click', () => this.handleYouTubeCapture());
        }
    ...
    /**
     * Handle YouTube thumbnail capture — uses Screen Capture API to grab a real
     * screenshot of the video (just like the 3D model screenshot workflow).
     * User pauses the video at the desired frame, clicks capture, browser asks
     * permission, we grab one frame, crop to square, show preview, upload on confirm.
     */
    async handleYouTubeCapture() {
        const exerciseId = this._ytExerciseId;
        const exerciseName = this._ytExerciseName;
        if (!exerciseId) return;

        // Check API availability
        if (!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia) {
            alert('Screen capture is not supported in this browser. Please use Chrome or Edge.');
            return;
        }

        let stream = null;
        try {
            // Prompt the user to share their screen/tab
            stream = await navigator.mediaDevices.getDisplayMedia({ video: true });
        } catch (err) {
            // User cancelled the permission dialog — silently bail out
            console.log('[YT Capture] User cancelled screen share:', err.message);
            return;
        }
        ...
        try {
            ...
            const sx = Math.floor((w - square) / 2);
            const sy = Math.floor((h - square) / 2);

            const cropCanvas = document.createElement('canvas');
            cropCanvas.width = 400;
            cropCanvas.height = 400;
            const cctx = cropCanvas.getContext('2d');
            cctx.drawImage(fullCanvas, sx, sy, square, square, 0, 0, 400, 400);

            const dataUrl = cropCanvas.toDataURL('image/jpeg', 0.92);

            // Show the preview modal
            this._showCapturePreviewModal(dataUrl, exerciseId, exerciseName);

        } catch (err) {
            if (stream) stream.getTracks().forEach(t => t.stop());
            console.error('[YT Capture] Error capturing frame:', err);
            alert('Failed to capture screenshot. Please try again.');
        }
    }

    /**
     * Show a modal with the captured screenshot preview and Confirm / Retake / Cancel buttons.
     */
    _showCapturePreviewModal(dataUrl, exerciseId, exerciseName) {
        let modal = document.getElementById('ffx-yt-capture-modal');
        if (modal) modal.remove();

        modal = document.createElement('div');
        modal.id = 'ffx-yt-capture-modal';
        modal.innerHTML = `
            <div class="ffx-capture-backdrop"></div>
            <div class="ffx-capture-dialog">
                <div class="ffx-capture-header">
                ...
                <p class="ffx-capture-subtitle">Use this screenshot for <strong>${exerciseName}</strong>?</p>
                <div class="ffx-capture-preview">
                    <img src="${dataUrl}" alt="Screenshot preview">
                </div>
                <p class="ffx-capture-note">The image will be saved as a 400×400 square to your Media Library.</p>
                <div class="ffx-capture-actions">
                    <button class="ffx-capture-cancel">Cancel</button>
                    <button class="ffx-capture-retake">
                    ...
                    <button class="ffx-capture-confirm">
                ...
                <div class="ffx-capture-status" style="display:none;"></div>
            </div>
        `;
        ...
        // Cancel
        modal.querySelector('.ffx-capture-cancel').addEventListener('click', () => modal.remove());
        modal.querySelector('.ffx-capture-backdrop').addEventListener('click', () => modal.remove());

        // Retake — close modal and re-invoke capture
        modal.querySelector('.ffx-capture-retake').addEventListener('click', () => {
            modal.remove();
            this.handleYouTubeCapture();
        });

        // Confirm — upload the base64 image to WordPress
        modal.querySelector('.ffx-capture-confirm').addEventListener('click', () => {
            const status = modal.querySelector('.ffx-capture-status');
            const confirmBtn = modal.querySelector('.ffx-capture-confirm');
            const retakeBtn = modal.querySelector('.ffx-capture-retake');

            confirmBtn.disabled = true;
            retakeBtn.disabled = true;
            confirmBtn.innerHTML = '<span class="ffx-capture-spinner"></span> Saving...';

            const ws = window.flexframeSettings || {};

            const formData = new FormData();
            formData.append('action', 'flexframe_capture_yt_thumbnail');
            formData.append('nonce', ws.ytCaptureNonce || '');
            formData.append('exercise_id', exerciseId);
            formData.append('image_data', dataUrl);

            fetch(ws.ajaxUrl, { method: 'POST', body: formData })
                .then(r => r.json())
                .then(resp => {
                    if (resp.success) {
                        modal.querySelector('.ffx-capture-actions').style.display = 'none';
                        status.style.display = 'block';
                        status.className = 'ffx-capture-status ffx-capture-success';
                        status.innerHTML = `
                            ...
                            <span>Thumbnail updated! It will appear next time the page loads.</span>
                        `;
                        // Live-update thumbnails in the current session
                        const thumbItems = document.querySelectorAll(`.thumbnail-item[data-id="${exerciseId}"] img`);
                        thumbItems.forEach(img => { img.src = resp.data.thumbnailUrl; });

                        setTimeout(() => modal.remove(), 3000);
                    } else {
                        confirmBtn.disabled = false;
                        retakeBtn.disabled = false;
                        confirmBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg> Save Thumbnail`;
                        status.style.display = 'block';
                        status.className = 'ffx-capture-status ffx-capture-error';
                        status.textContent = 'Error: ' + (resp.data || 'Failed to save thumbnail');
                    }
                })
                .catch(() => {
                    confirmBtn.disabled = false;
