...
class ThreeJSApp {
    ...
    setupGUI() {
        ...
        // Geometry particle system controls
        this.setupGeometryParticlesGUI();
    ...
    setupGeometryParticlesGUI() {
        const geometryFolder = this.gui.addFolder('🧬 Geometry System');
        const geometryParams = this.geometrySystem.getParams();

        // System Controls
        geometryFolder.add(geometryParams, 'visible').name('Visible')
            .onChange((value) => this.geometrySystem.setVisible(value));

        // Basic Particle Controls
        const particleFolder = geometryFolder.addFolder('Background Particles');
        
        particleFolder.add(geometryParams, 'count', 50, 1000, 10).name('Particle Count')
            .onChange((value) => this.geometrySystem.updateParticleCount(value));

        particleFolder.add(geometryParams, 'size', 0.001, 0.01, 0.0001).name('Particle Size')
            .onChange((value) => this.geometrySystem.updateParticleSize(value));

        particleFolder.addColor(geometryParams, 'color').name('Particle Color')
            .onChange((value) => this.geometrySystem.updateParticleColor(value));

        particleFolder.add(geometryParams, 'opacity', 0, 0.5, 0.01).name('Particle Opacity')
            .onChange((value) => this.geometrySystem.updateParticleOpacity(value));

        particleFolder.add(geometryParams, 'speed', 0, 1, 0.1).name('Movement Speed')
            .onChange((value) => this.geometrySystem.updateSpeed(value));

        particleFolder.add(geometryParams, 'depthOffset', -3, 1, 0.1).name('Depth Position')
            .onChange(() => this.geometrySystem.createParticles());

        // Connection System Controls
        const connectionFolder = geometryFolder.addFolder('🧬 Connection System');
        
        connectionFolder.add(geometryParams, 'connectionsEnabled').name('Enable Connections')
            .onChange((value) => this.geometrySystem.toggleConnections(value));
            
        connectionFolder.add(geometryParams, 'connectionCount', 50, 500, 10).name('Connection Count')
            .onChange((value) => this.geometrySystem.updateConnectionCount(value));
            
        connectionFolder.add(geometryParams, 'connectionDensity', 0, 1, 0.01).name('Connection Density')
            .onChange((value) => this.geometrySystem.updateConnectionDensity(value));
            
        connectionFolder.add(geometryParams, 'connectionDistance', 0.5, 4, 0.1).name('Max Distance')
            .onChange((value) => this.geometrySystem.updateConnectionDistance(value));
            
        connectionFolder.addColor(geometryParams, 'connectionColor').name('Connection Color')
            .onChange((value) => this.geometrySystem.updateConnectionColor(value));
            
        connectionFolder.add(geometryParams, 'connectionOpacity', 0, 0.5, 0.01).name('Connection Opacity')
            .onChange((value) => this.geometrySystem.updateConnectionOpacity(value));
            
        connectionFolder.add(geometryParams, 'connectionSpeed', 0, 1, 0.1).name('Animation Speed')
            .onChange((value) => this.geometrySystem.updateConnectionSpeed(value));
            
        connectionFolder.add(geometryParams, 'connectionWidth', 1, 3, 1).name('Line Width');
            
        connectionFolder.add(geometryParams, 'connectionPattern', 
            ['random', 'nearest', 'dna', 'chemical']).name('Pattern Type')
            .onChange((value) => this.geometrySystem.updateConnectionPattern(value));
            
        // Connection Pattern Presets
        const geometryPresets = {
            '🧬 DNA Strands': () => {
                geometryParams.connectionsEnabled = true;
                geometryParams.connectionPattern = 'dna';
                geometryParams.connectionCount = 200;
                geometryParams.connectionDensity = 0.3;
                geometryParams.connectionColor = '#00ff88';
                geometryParams.connectionOpacity = 0.08;
                geometryParams.connectionSpeed = 0.2;
                this.geometrySystem.toggleConnections(true);
                this.geometrySystem.updateConnectionPattern('dna');
                this.geometrySystem.updateConnectionCount(200);
                this.geometrySystem.updateConnectionDensity(0.3);
                this.geometrySystem.updateConnectionColor('#00ff88');
                this.geometrySystem.updateConnectionOpacity(0.08);
                this.geometrySystem.updateConnectionSpeed(0.2);
                connectionFolder.controllersRecursive().forEach(c => c.updateDisplay());
            },
            '⚗️ Chemical Bonds': () => {
                geometryParams.connectionsEnabled = true;
                geometryParams.connectionPattern = 'chemical';
                geometryParams.connectionCount = 150;
                geometryParams.connectionDensity = 0.25;
                geometryParams.connectionColor = '#ffaa00';
                geometryParams.connectionOpacity = 0.1;
                geometryParams.connectionDistance = 1.5;
                this.geometrySystem.toggleConnections(true);
                this.geometrySystem.updateConnectionPattern('chemical');
                this.geometrySystem.updateConnectionCount(150);
                this.geometrySystem.updateConnectionDensity(0.25);
                this.geometrySystem.updateConnectionColor('#ffaa00');
                this.geometrySystem.updateConnectionOpacity(0.1);
                this.geometrySystem.updateConnectionDistance(1.5);
                connectionFolder.controllersRecursive().forEach(c => c.updateDisplay());
            },
            '🕸️ Neural Network': () => {
                geometryParams.connectionsEnabled = true;
                geometryParams.connectionPattern = 'nearest';
                geometryParams.connectionCount = 300;
                geometryParams.connectionDensity = 0.4;
                geometryParams.connectionColor = '#00aaff';
                geometryParams.connectionOpacity = 0.06;
                geometryParams.connectionSpeed = 0.1;
                this.geometrySystem.toggleConnections(true);
                this.geometrySystem.updateConnectionPattern('nearest');
                this.geometrySystem.updateConnectionCount(300);
                this.geometrySystem.updateConnectionDensity(0.4);
                this.geometrySystem.updateConnectionColor('#00aaff');
                this.geometrySystem.updateConnectionOpacity(0.06);
                this.geometrySystem.updateConnectionSpeed(0.1);
                connectionFolder.controllersRecursive().forEach(c => c.updateDisplay());
            },
            '❌ Disable Geometry': () => {
                geometryParams.visible = false;
                geometryParams.connectionsEnabled = false;
                this.geometrySystem.setVisible(false);
                this.geometrySystem.toggleConnections(false);
                geometryFolder.controllersRecursive().forEach(c => c.updateDisplay());
            }
        };
        
        connectionFolder.add(geometryPresets, '🧬 DNA Strands').name('🧬 DNA Strands');
        connectionFolder.add(geometryPresets, '⚗️ Chemical Bonds').name('⚗️ Chemical Bonds');
        connectionFolder.add(geometryPresets, '🕸️ Neural Network').name('🕸️ Neural Network');
        connectionFolder.add(geometryPresets, '❌ Disable Geometry').name('❌ Disable Geometry');
