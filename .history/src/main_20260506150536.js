...
class ThreeJSApp {
    ...
    async recordPresetVideo(presetKey) {
        ...
        // 30-second countdown
        let countdownSec = 30;
        if (status) status.textContent = `Recording… ${countdownSec}s`;
        const countdownTimer = setInterval(() => {
            countdownSec--;
            if (countdownSec > 0 && status) status.textContent = `Recording… ${countdownSec}s`;
        }, 1000);
        ...
        try {
            ...
            clearInterval(countdownTimer);
            if (status) { status.classList.add('error'); status.textContent = error.message || 'Recording failed.'; }
        } finally {
