// ---------------------------------------------
// 5b. Spotlights: Add two spotlights with helpers
const spotLights = [];
const spotHelpers = [];
const spotParams = [
    {
...
// GUI controls for spotlights
const spotFolder = gui.addFolder('Spotlights');
spotLights.forEach((spot, i) => {
    const p = spotParams[i];
    const f = spotFolder.addFolder('Spotlight ' + (i + 1));
    f.addColor(p, 'color').name('Color').onChange((v) => {
        spot.color.set(v);
        p.color = v;
    });
    f.add(p, 'intensity', 0, 5, 0.01).name('Intensity').onChange((v) => {
        spot.intensity = v;
    });
    f.add(spot.position, 'x', -10, 10, 0.01).name('Pos X');
    f.add(spot.position, 'y', 0, 10, 0.01).name('Pos Y');
    f.add(spot.position, 'z', -10, 10, 0.01).name('Pos Z');
    f.add(p, 'angle', 0.01, Math.PI / 2, 0.01).name('Angle').onChange((v) => {
        spot.angle = v;
    });
    f.add(p, 'penumbra', 0, 1, 0.01).name('Penumbra').onChange((v) => {
        spot.penumbra = v;
    });
    f.add(p, 'visible').name('Visible').onChange((v) => {
        spot.visible = v;
        spotHelpers[i].visible = v;
    });
    f.open();
});
spotFolder.open();
...
const defaultSettings = {
    ...
    camera: {
        ...
        target: [
            0.008201261378768233,
            0.4233815259901379,
            0.0068031253433533225
        ]
    }
};
