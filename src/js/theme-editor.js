...
class ThemeEditor {
    ...
    loadCurrentSettings() {
        ...
        this.currentSettings = {
            ...
            menuV2InfoPanelOpacity: ws.uiSettings?.menuV2?.infoPanelOpacity ?? 0.95,
                        ...
                        <div class="te-section te-nested">
                            ...
                            <div class="te-section-content" id="section-ui-menu-v2">
                                ...
                                <div style="border-top: 1px solid rgba(255,255,255,0.1); margin: 12px 0 8px; padding-top: 8px;">
                                    <div style="font-size: 11px; font-weight: 600; color: rgba(255,255,255,0.7); margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px;">Search Menu (Left)</div>
                                </div>
                                ${this.createColorInput('menuV2HeadingBgColor', 'Header BG Color')}
                                ${this.createRangeInput('menuV2HeadingBgOpacity', 'Header & Border Opacity', 0, 1, 0.01)}
                                <div style="border-top: 1px solid rgba(255,255,255,0.1); margin: 12px 0 8px; padding-top: 8px;">
                                    <div style="font-size: 11px; font-weight: 600; color: rgba(255,255,255,0.7); margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px;">Info Panel (Right)</div>
                                </div>
                                ${this.createRangeInput('menuV2InfoPanelOpacity', 'Header & Border Opacity', 0, 1, 0.01)}
                                ${this.createRangeInput('menuV2InfoHeaderOpacity', 'Section Header BG Opacity', 0, 1, 0.01)}
                                <div style="border-top: 1px solid rgba(255,255,255,0.1); margin: 12px 0 8px; padding-top: 8px;">
    ...
    applySettingLive(key, value) {
        ...
        if (matchedEquipment && window.model) {
            ...
            if (matchedEquipment === 'color1' && key === 'color1Color') {
                ...
                this.updateHDMaterialColor('BUMPER', value);
                this.updateHDMaterialColor('XBUMPER', value);
            }
    ...
    applyPrimaryColorToElements(color) {
        ...
        // 5. Update all primary-derived settings BEFORE calling updateMenuStyling
        this.currentSettings.menuV2AccentColor = color;
        const accentInput = this.panel.querySelector('[data-setting="menuV2AccentColor"]');
        if (accentInput) {
            accentInput.value = color;
            const hexDisplay = this.panel.querySelector('[data-hex-for="menuV2AccentColor"]');
            if (hexDisplay) hexDisplay.textContent = color;
        }
        
        this.currentSettings.menuV2HeadingBgColor = color;
        const headingBgInput = this.panel.querySelector('[data-setting="menuV2HeadingBgColor"]');
        if (headingBgInput) {
            headingBgInput.value = color;
            const hexDisplay = this.panel.querySelector('[data-hex-for="menuV2HeadingBgColor"]');
            if (hexDisplay) hexDisplay.textContent = color;
        }
        
        // 6. Update Particle color to match primary
        this.currentSettings.particlesColor = color;
        const particleInput = this.panel.querySelector('[data-setting="particlesColor"]');
        if (particleInput) {
            particleInput.value = color;
            const hexDisplay = this.panel.querySelector('[data-hex-for="particlesColor"]');
            if (hexDisplay) hexDisplay.textContent = color;
        }
        ...
        // 7. Update Color1 (equipment accent) material to match primary
        this.currentSettings.color1Color = color;
        const color1Input = this.panel.querySelector('[data-setting="color1Color"]');
        if (color1Input) {
    ...
    updateXClothesMaterialColor(color) {
        ...
        // XCLOTHES tracks the primary color; AIBODYGIRL always stays white.
        window.model.traverse((child) => {
            if (child.isMesh && child.material) {
                const materials = Array.isArray(child.material) ? child.material : [child.material];
                ...
                materials.forEach(mat => {
                    if (!mat.name) return;
                    const upper = mat.name.toUpperCase();
                    if (upper === 'XCLOTHES') {
                        mat.color.set(color);
                        mat.needsUpdate = true;
                    } else if (upper === 'AIBODYGIRL') {
    ...
    updateMenuStyling() {
        ...
        const leftBorderRgba = this.hexToRgba(accentColor, headingBgOpacity);
        const infoPanelOpacity = this.currentSettings.menuV2InfoPanelOpacity ?? headingBgOpacity;
        const rightBorderRgba = this.hexToRgba(accentColor, infoPanelOpacity);
        const infoStickyHeaderRgba = this.hexToRgba(headingBgColor, infoPanelOpacity);
        ...
        styleEl.textContent = `
            ...
            /* ALL LEFT dropdown borders + box-shadow by ID (accent color with heading opacity) */
            #flexframe-viewer-container #searchDropdown,
            #flexframe-viewer-container #exercisesDropdown,
            #flexframe-viewer-container #musclesDropdown,
            #flexframe-viewer-container #equipmentDropdown,
            ...
            #flexframe-viewer-container .thumbnail-dropdown,
            .thumbnail-dropdown {
                border: 2px solid ${leftBorderRgba} !important;
            ...
            #searchDropdown.thumbnail-dropdown.show {
                border-color: ${leftBorderRgba} !important;
                box-shadow: 0 0 20px ${accentColor}66, 0 0 40px ${accentColor}33 !important;
            }
            ...
            .thumbnail-dropdown-right {
                background-color: ${bgRgba} !important;
                border: 2px solid ${rightBorderRgba} !important;
            }
            ...
            /* Search Input - use side menu V2 background color with own opacity */
            ...
            .search-header .search-input {
                background: ${searchInputBgRgba} !important;
                color: ${textRgba} !important;
            ...
            .thumbnail-dropdown .search-input:focus {
                background: ${searchInputBgFocusRgba} !important;
                border-color: ${accentColor} !important;
            }
    ...
    exportTheme() {
        const themeName = this.panel.querySelector('#te-theme-name').value.trim() || 'flexframe-theme';
        const exportData = {
            name: themeName,
            version: '1.0',
        ...
        a.href = url;
        a.download = `${themeName.replace(/[^a-z0-9_-]/gi, '_')}.json`;
    ...
    async saveTheme() {
        ...
        const presetData = {
            ...
            menu_v2_info_panel_opacity: this.currentSettings.menuV2InfoPanelOpacity,
        ...
        try {
            ...
            if (window.flexframeSettings && window.flexframeSettings.ajaxUrl) {
                ...
                if (result.success) {
                    this.showMessage(`Theme "${themeName}" saved & applied! Settings will persist on reload.`, 'success');
