...
(function() {
    ...
    async function loadExerciseCatalogue() {
        try {
            ...
            if (Array.isArray(SETTINGS.customExercises) && SETTINGS.customExercises.length > 0) {
                ...
                console.log(`[Workout Builder] Merged ${SETTINGS.customExercises.length} custom exercises`);
            }
    ...
    function assignExerciseToCard(uid, catalogueExercise) {
        ...
        card.youtubeUrl = catalogueExercise.youtubeUrl || '';
        closeFinder();
        renderExerciseList();
    }
        ...
        finderResultsGrid.innerHTML = results.map(ex => `
            <div class="ffwb-finder-item${ex.source === 'custom' ? ' ffwb-finder-item--custom' : ''}" data-exercise-id="${ex.id}">
                <div class="ffwb-finder-item-thumb">
                    ${ex.thumbnailUrl
                        ? `<img${ex.source === 'custom' ? ' style="filter:grayscale(1)!important"' : ''} src="${ex.thumbnailUrl}" alt="${ex.name}" loading="lazy">`
    ...
    function createCardElement(exercise, number, subLabel, groupId) {
            ...
            <div class="ffwb-card-inner ffwb-swipe-content">
                            ...
                            <div class="ffwb-flip-front">
                                ${isUnassigned
                                    ...
                                    : exercise.thumbnailUrl 
                                        ? `<img${exercise.source === 'custom' ? ' style="filter:grayscale(1)!important"' : ''} src="${exercise.thumbnailUrl}" alt="${exercise.name}" loading="lazy">`
                ...
                <div class="ffwb-card-body">
                    ...
                    <div class="ffwb-card-expand-row">
                        ${!isUnassigned && exercise.exerciseId ? `<a class="ffwb-btn-learn3d" href="#" data-exercise-id="${exercise.exerciseId}"${exercise.youtubeUrl ? ' data-youtube-url="' + exercise.youtubeUrl + '"' : ''} target="_blank" rel="noopener noreferrer" title="${exercise.source === 'custom' ? 'Watch exercise video' : 'Learn how to perform this exercise in 3D'}">
        ...
        if (!isReadOnly) {
            ...
            // Learn in 3D / Watch Video link
            card.querySelector('.ffwb-btn-learn3d')?.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                const ytUrl = e.currentTarget.dataset.youtubeUrl;
                if (ytUrl) {
                    // Custom exercise — open YouTube video directly
                    window.open(ytUrl, '_blank');
                } else {
                    // Standard exercise — open in 3D viewer
    ...
    async function saveWorkout(visibility = 'private') {
        ...
        // Validate REST URL exists (page caching can strip wp_localize_script data)
        if (!SETTINGS.restUrl) {
            console.error('[Workout Builder] REST URL missing — SETTINGS:', SETTINGS);
            showToast('Connection error — please refresh the page and try again');
            return;
        }
        ...
        try {
            ...
            if (!SETTINGS.isLoggedIn && visibility === 'public') {
                ...
                url = SETTINGS.restUrl + 'workouts/share';
                opts = {
                    method: 'POST',
                ...
                // Update existing
                url = SETTINGS.restUrl + 'workouts/' + workoutId;
                opts = {
                    method: 'PUT',
                    credentials: 'same-origin',
                    headers: {
            ...
            console.log('[Workout Builder] Saving to:', url, 'method:', opts.method);
            const res = await fetch(url, opts);

            // Check Content-Type before parsing — servers may return HTML error pages
            const ct = (res.headers.get('content-type') || '');
            if (!res.ok) {
                const body = ct.includes('json') ? await res.json() : await res.text();
                const msg = (typeof body === 'object' && body.message) ? body.message
                          : (res.status === 401 || res.status === 403) ? 'Session expired — please refresh the page and try again'
                          : 'Server returned ' + res.status;
                console.error('[Workout Builder] Save failed:', res.status, body);
                throw new Error(msg);
            }

            if (!ct.includes('json')) {
                const body = await res.text();
                console.error('[Workout Builder] Expected JSON but got:', ct, body.substring(0, 200));
    ...
    async function downloadWorkoutPDF() {
        ...
        for (const group of groups) {
            ...
            for (let gi = 0; gi < group.length; gi++) {
                ...
                // clickable "Learn in 3D" / "Watch Video" link under QR (or at right if no QR)
                if (exUrl) {
                    const isCustom = exercise.source === 'custom';
                    const linkText = isCustom ? 'Watch Video \u2192' : 'Learn in 3D \u2192';
                    doc.setFontSize(7);
                    doc.setFont(undefined, 'bold');
