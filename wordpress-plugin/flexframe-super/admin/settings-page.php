...
function flexframe_register_settings() {
    ...
    // ========== AI Social Media Generator Settings ==========
    register_setting('flexframe_settings_group', 'flexframe_ai_prompt_template', array(
        'type' => 'string',
        'sanitize_callback' => 'wp_kses_post',
        'default' => flexframe_ai_default_prompt_template()
    ));
    register_setting('flexframe_settings_group', 'flexframe_ai_caption_template', array(
        'type' => 'string',
        'sanitize_callback' => 'wp_kses_post',
        'default' => flexframe_ai_default_caption_template()
    ));
    register_setting('flexframe_settings_group', 'flexframe_ai_reference_logo_url', array(
        'type' => 'string',
        'sanitize_callback' => 'esc_url_raw',
        'default' => ''
    ));
    register_setting('flexframe_settings_group', 'flexframe_ai_reference_theme_url', array(
        'type' => 'string',
        'sanitize_callback' => 'esc_url_raw',
        'default' => ''
    ));
    register_setting('flexframe_settings_group', 'flexframe_ai_reference_person_url', array(
        'type' => 'string',
        'sanitize_callback' => 'esc_url_raw',
        'default' => ''
    ));
    register_setting('flexframe_settings_group', 'flexframe_ai_use_references', array(
        'type' => 'boolean',
        'sanitize_callback' => 'rest_sanitize_boolean',
        'default' => true
    ));
    ...
    // ── AI Coach (Step 9 — chat assistant) ──
    register_setting('flexframe_settings_group', 'flexframe_ai_coach_enabled', array(
        'type' => 'boolean', 'sanitize_callback' => 'rest_sanitize_boolean', 'default' => true
    ));
    register_setting('flexframe_settings_group', 'flexframe_ai_coach_public', array(
    ...
    register_setting('flexframe_settings_group', 'flexframe_ai_image_logged_in_enabled', array(
        'type' => 'boolean', 'sanitize_callback' => 'rest_sanitize_boolean', 'default' => true
    ));
    ...
    register_setting('flexframe_settings_group', 'flexframe_ai_workout_image_prompt_template', array(
        'type' => 'string', 'sanitize_callback' => 'wp_kses_post', 'default' => ''
    ));
    register_setting('flexframe_settings_group', 'flexframe_ai_workout_caption_template', array(
        'type' => 'string', 'sanitize_callback' => 'wp_kses_post', 'default' => ''
    ));
            ...
            <form method="post" action="options.php" id="flexframe-settings-form">
                ...
                <div class="flexframe-step-section flexframe-step-section--page-tool collapsed">
                    ...
                    <div class="flexframe-step-content" style="display: none;">
                        ...
                        <!-- ── Sub-panel: AI Coach (chat assistant) ── -->
                        <div class="custom-panel-section">
                            <div class="custom-panel-header" data-target="workout-ai-coach-content">
                                <h4><span class="dashicons dashicons-format-chat"></span> <?php _e('AI Coach (Chat Assistant)', 'flexframe-viewer'); ?></h4>
                                <span class="toggle-icon dashicons dashicons-arrow-down-alt2"></span>
                            </div>
                            <div class="custom-panel-content" id="workout-ai-coach-content">
                                <p style="margin-top: 0; color: #666; font-size: 13px;">
                                    <?php _e('Configure the floating AI Coach widget on the Workout Builder page. The coach uses your gym\'s exercise catalogue to generate personalised single-session workouts via OpenAI.', 'flexframe-viewer'); ?>
                                </p>
                                ...
                                <div class="notice notice-warning inline" style="margin: 0 0 16px; padding: 8px 12px;">
                                    ...
                                    <p style="margin: 4px 0;"><code>define('FLEXFRAME_OPENAI_KEY', 'sk-...');</code></p>
                                </div>
                                <?php endif; ?>

                                <table class="form-table" role="presentation">
                                    <tr>
                                        <th scope="row"><?php _e('Enable AI Coach', 'flexframe-viewer'); ?></th>
                                        <td>
                                            <label><input type="hidden" name="flexframe_ai_coach_enabled" value="0"><input type="checkbox" name="flexframe_ai_coach_enabled" value="1" <?php checked($ai_enabled); ?>>
                                                <?php _e('Show the AI Coach floating bubble on the Workout Builder page.', 'flexframe-viewer'); ?></label>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <label><input type="hidden" name="flexframe_ai_coach_public" value="0"><input type="checkbox" name="flexframe_ai_coach_public" value="1" <?php checked($ai_public); ?>>
                                                <?php _e('Allow anonymous (logged-out) visitors to use the AI Coach.', 'flexframe-viewer'); ?></label>
                                            <p class="description" style="color: #b32d2e;"><?php _e('⚠ Warning: Public access can rapidly increase your OpenAI bill. Rate limits still apply per IP, but anyone can hit the endpoint. Leave OFF unless you trust your audience.', 'flexframe-viewer'); ?></p>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row"><?php _e('Free-form chat', 'flexframe-viewer'); ?></th>
                                        <td>
                                            <label><input type="hidden" name="flexframe_ai_coach_chat_enabled" value="0"><input type="checkbox" name="flexframe_ai_coach_chat_enabled" value="1" <?php checked($ai_chat_enabled); ?>>
                                                <?php _e('Allow users to type free-form messages to the coach.', 'flexframe-viewer'); ?></label>
                                            <p class="description"><?php _e('When OFF, users can only use the intake form + Generate Now button (no typing). Reduces token use and abuse surface.', 'flexframe-viewer'); ?></p>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <label><input type="hidden" name="flexframe_ai_coach_wod_enabled" value="0"><input type="checkbox" name="flexframe_ai_coach_wod_enabled" value="1" <?php checked($ai_wod_enabled); ?>>
                                                <?php _e('Show the WOD button (one-click randomised workout, ignores user profile).', 'flexframe-viewer'); ?></label>
                                    ...
                                    <tr>
                                        <th scope="row"><?php _e('AI Image button (logged-in)', 'flexframe-viewer'); ?></th>
                                        <td>
                                            <label><input type="hidden" name="flexframe_ai_image_logged_in_enabled" value="0"><input type="checkbox" name="flexframe_ai_image_logged_in_enabled" value="1" <?php checked($ai_image_li); ?>>
                                                <?php _e('Show the “A.I Image” button to logged-in users.', 'flexframe-viewer'); ?></label>
                                            <p class="description"><?php _e('Placeholder action button in the coach toolbar (replaces the old Chat button). Future feature.', 'flexframe-viewer'); ?></p>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row"><?php _e('AI Image button (logged-out)', 'flexframe-viewer'); ?></th>
                                        <td>
                                ...
                                $wo_img_default = function_exists('flexframe_ai_default_workout_image_prompt_template') ? flexframe_ai_default_workout_image_prompt_template() : '';
                                $wo_cap_default = function_exists('flexframe_ai_default_workout_caption_template') ? flexframe_ai_default_workout_caption_template() : '';
                                if ($wo_img_tpl === '') { $wo_img_tpl = $wo_img_default; }
                                if ($wo_cap_tpl === '') { $wo_cap_tpl = $wo_cap_default; }
                                ?>
                                <h4 style="margin-top: 24px; margin-bottom: 6px;"><?php _e('A.I Image — Workout Infographic Prompts', 'flexframe-viewer'); ?></h4>
                                <p class="description" style="margin: 0 0 12px 0;">
                                    <?php _e('When a user clicks the <strong>A.I Image</strong> button in the Workout Builder, the current workout is screenshotted and sent to GPT-Image-2 with the prompt below to produce a 1:1 Instagram infographic. The Brand Logo and Social Media Theme references from <em>Step 11 — AI Social Media Generator</em> are also sent so the image matches your gym\'s look.', 'flexframe-viewer'); ?>
                                </p>

                                <div class="flexframe-setting-row" style="flex-direction: column; align-items: stretch; margin-top: 8px;">
                                    <label for="flexframe_ai_workout_image_prompt_template" style="margin-bottom: 6px;">
                                        <strong><?php _e('Workout Image Prompt Template', 'flexframe-viewer'); ?></strong>
                                    </label>
                                    <p class="description" style="margin: 0 0 8px 0;">
                                        <?php _e('Placeholders: <code>{gymName}</code>, <code>{workoutName}</code>, <code>{exerciseList}</code>.', 'flexframe-viewer'); ?>
                                    </p>
                                    <textarea
                                        id="flexframe_ai_workout_image_prompt_template"
                                        name="flexframe_ai_workout_image_prompt_template"
                                        rows="10"
                                        class="large-text code"
                                        style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 13px;"
                                    ><?php echo esc_textarea($wo_img_tpl); ?></textarea>
                                    <div style="margin-top: 6px;">
                                        <button type="button" class="button button-secondary" id="flexframe_ai_reset_workout_prompt"
                                                data-default="<?php echo esc_attr($wo_img_default); ?>">
                                ...
                                <div class="flexframe-setting-row" style="flex-direction: column; align-items: stretch; margin-top: 16px;">
                                    <label for="flexframe_ai_workout_caption_template" style="margin-bottom: 6px;">
                                    ...
                                    <textarea
                                        ...
                                        name="flexframe_ai_workout_caption_template"
                                        rows="8"
                                        class="large-text code"
                                        style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 13px;"
                                    ><?php echo esc_textarea($wo_cap_tpl); ?></textarea>
                                    <div style="margin-top: 6px;">
                                        <button type="button" class="button button-secondary" id="flexframe_ai_reset_workout_caption"
                                                data-default="<?php echo esc_attr($wo_cap_default); ?>">
                                            <?php _e('Reset to Default', 'flexframe-viewer'); ?>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <script>
                                jQuery(function($){
                                    $('#flexframe_ai_reset_workout_prompt').on('click', function(){
                                        $('#flexframe_ai_workout_image_prompt_template').val($(this).data('default'));
                                    });
                                    $('#flexframe_ai_reset_workout_caption').on('click', function(){
                                        $('#flexframe_ai_workout_caption_template').val($(this).data('default'));
                                    });
                                });
                                </script>
                ...
                <div class="flexframe-step-section collapsed">
                    <div class="flexframe-step-header" data-step="11">
                        ...
                        <h2><?php _e('AI Social Media Generator', 'flexframe-viewer'); ?></h2>
                        <span class="step-toggle-icon dashicons dashicons-arrow-down-alt2"></span>
                        <button type="submit" class="button button-primary step-save-btn">
                            <span class="dashicons dashicons-saved"></span><?php _e('Save Settings', 'flexframe-viewer'); ?>
                        </button>
                    ...
                    <div class="flexframe-step-content" style="display: none;">
                        <p class="step-description">
                            <?php _e('Customize the AI prompt and provide reference images that the AI will use when generating your branded social media posts. Reference images are optional — they help the AI match your brand style, logo, and athlete look.', 'flexframe-viewer'); ?>
                        </p>
                        ...
                        <div class="flexframe-setting-row" style="flex-direction: column; align-items: stretch;">
                            ...
                            <textarea
                                id="flexframe_ai_prompt_template"
                                name="flexframe_ai_prompt_template"
                                rows="8"
                                class="large-text code"
                                style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 13px;"
                            ><?php echo esc_textarea($ai_prompt_tpl); ?></textarea>
                            <div style="margin-top: 6px;">
                        ...
                        <!-- Use reference images toggle -->
                        <div class="flexframe-setting-row" style="margin-top: 16px;">
                            <label class="toggle-switch" style="flex-shrink: 0;">
                                <input type="hidden" name="flexframe_ai_use_references" value="0">
                                <input type="checkbox" id="flexframe_ai_use_references" name="flexframe_ai_use_references" value="1" <?php checked($ai_use_references); ?>>
                                <span class="toggle-slider"></span>
                            </label>
                            <div style="display:flex; flex-direction:column; gap:2px; margin-left: 8px;">
                                <strong><?php _e('Send Reference Images to AI', 'flexframe-viewer'); ?></strong>
                                <span class="description" style="font-size:12px; color:#757575;"><?php _e('When enabled, any uploaded references below are sent alongside the screenshot.', 'flexframe-viewer'); ?></span>
                            </div>
                        </div>

                        <!-- Reference images grid -->
                        <div class="flexframe-ai-references" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 16px; margin-top: 16px;">

                            <!-- Brand Logo -->
                            <div class="flexframe-ai-ref-card" style="border:1px solid #ccd0d4; border-radius:8px; padding:12px; background:#fafafa;">
                                <strong style="display:block; margin-bottom:6px;"><?php _e('Brand Logo', 'flexframe-viewer'); ?></strong>
                                <p class="description" style="margin:0 0 8px 0; font-size:12px;"><?php _e('PNG with transparent background recommended.', 'flexframe-viewer'); ?></p>
                                <input type="hidden" id="flexframe_ai_reference_logo_url" name="flexframe_ai_reference_logo_url" value="<?php echo esc_attr($ai_ref_logo); ?>">
                                <div class="ai-ref-preview" data-target="flexframe_ai_reference_logo_url" style="min-height:100px; background:#fff; border:1px dashed #ccd0d4; border-radius:6px; display:flex; align-items:center; justify-content:center; margin-bottom:8px;">
                                    <?php if ($ai_ref_logo) : ?>
                                        <img src="<?php echo esc_url($ai_ref_logo); ?>" style="max-width:100%; max-height:120px;">
                                    <?php else : ?>
                                        <span style="color:#999; font-size:12px;"><?php _e('No image selected', 'flexframe-viewer'); ?></span>
                                    <?php endif; ?>
                                </div>
                                <div style="display:flex; gap:6px;">
                                    <button type="button" class="button button-primary flexframe-ai-upload" data-target="flexframe_ai_reference_logo_url" data-title="<?php esc_attr_e('Select Brand Logo', 'flexframe-viewer'); ?>">
                                        <?php _e('Upload', 'flexframe-viewer'); ?>
                                    </button>
                                    <button type="button" class="button flexframe-ai-remove" data-target="flexframe_ai_reference_logo_url" <?php echo empty($ai_ref_logo) ? 'style="display:none;"' : ''; ?>>
                                        <?php _e('Remove', 'flexframe-viewer'); ?>
                                    </button>
                                </div>
                            </div>
                            ...
                            <!-- Social Media Theme -->
                            <div class="flexframe-ai-ref-card" style="border:1px solid #ccd0d4; border-radius:8px; padding:12px; background:#fafafa;">
                                <strong style="display:block; margin-bottom:6px;"><?php _e('Social Media Theme', 'flexframe-viewer'); ?></strong>
                                <p class="description" style="margin:0 0 8px 0; font-size:12px;"><?php _e('A sample post showing your style, colors, and typography.', 'flexframe-viewer'); ?></p>
                                <input type="hidden" id="flexframe_ai_reference_theme_url" name="flexframe_ai_reference_theme_url" value="<?php echo esc_attr($ai_ref_theme); ?>">
                                <div class="ai-ref-preview" data-target="flexframe_ai_reference_theme_url" style="min-height:100px; background:#fff; border:1px dashed #ccd0d4; border-radius:6px; display:flex; align-items:center; justify-content:center; margin-bottom:8px;">
                                    <?php if ($ai_ref_theme) : ?>
                                        <img src="<?php echo esc_url($ai_ref_theme); ?>" style="max-width:100%; max-height:120px;">
                                    <?php else : ?>
                                        <span style="color:#999; font-size:12px;"><?php _e('No image selected', 'flexframe-viewer'); ?></span>
                                    <?php endif; ?>
                                </div>
                                <div style="display:flex; gap:6px;">
                                    <button type="button" class="button button-primary flexframe-ai-upload" data-target="flexframe_ai_reference_theme_url" data-title="<?php esc_attr_e('Select Theme Reference', 'flexframe-viewer'); ?>">
                                        <?php _e('Upload', 'flexframe-viewer'); ?>
                                    </button>
                                    <button type="button" class="button flexframe-ai-remove" data-target="flexframe_ai_reference_theme_url" <?php echo empty($ai_ref_theme) ? 'style="display:none;"' : ''; ?>>
                                        <?php _e('Remove', 'flexframe-viewer'); ?>
                                    </button>
                                </div>
                            </div>
                            ...
                            <!-- Reference Athlete -->
                            <div class="flexframe-ai-ref-card" style="border:1px solid #ccd0d4; border-radius:8px; padding:12px; background:#fafafa;">
                                <strong style="display:block; margin-bottom:6px;"><?php _e('Reference Athlete', 'flexframe-viewer'); ?></strong>
                                <p class="description" style="margin:0 0 8px 0; font-size:12px;"><?php _e('A photo whose look/build inspires the figure in the post. Note: AI uses this as style inspiration, not exact face swap.', 'flexframe-viewer'); ?></p>
                                <input type="hidden" id="flexframe_ai_reference_person_url" name="flexframe_ai_reference_person_url" value="<?php echo esc_attr($ai_ref_person); ?>">
                                <div class="ai-ref-preview" data-target="flexframe_ai_reference_person_url" style="min-height:100px; background:#fff; border:1px dashed #ccd0d4; border-radius:6px; display:flex; align-items:center; justify-content:center; margin-bottom:8px;">
                                    <?php if ($ai_ref_person) : ?>
                                        <img src="<?php echo esc_url($ai_ref_person); ?>" style="max-width:100%; max-height:120px;">
                                    <?php else : ?>
                                        <span style="color:#999; font-size:12px;"><?php _e('No image selected', 'flexframe-viewer'); ?></span>
                                    <?php endif; ?>
                                </div>
                                <div style="display:flex; gap:6px;">
                                    <button type="button" class="button button-primary flexframe-ai-upload" data-target="flexframe_ai_reference_person_url" data-title="<?php esc_attr_e('Select Reference Athlete', 'flexframe-viewer'); ?>">
                                        <?php _e('Upload', 'flexframe-viewer'); ?>
                                    </button>
                                    <button type="button" class="button flexframe-ai-remove" data-target="flexframe_ai_reference_person_url" <?php echo empty($ai_ref_person) ? 'style="display:none;"' : ''; ?>>
                                        <?php _e('Remove', 'flexframe-viewer'); ?>
                                    </button>
                                </div>
                            </div>
                        ...
                        <p class="description" style="margin-top: 16px;">
                            <?php _e('Tip: results vary per AI provider. Gemini (Nano Banana) tends to follow theme references closely; OpenAI gpt-image-2 is generally stronger on text and typography.', 'flexframe-viewer'); ?>
                        </p>
                    </div>
                </div>
                <!-- End Step 11 -->
